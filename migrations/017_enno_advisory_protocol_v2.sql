-- Forward-only protocol eligibility: add the verify_prepare operation receipt
-- value and require SQLite integer storage type (not merely column affinity)
-- for the advisory revision/rank/version/degraded columns. Migration 016 is
-- left byte-identical; valid rows are preserved atomically.

ALTER TABLE enno_operation_receipts RENAME TO enno_operation_receipts_v16;
...
SELECT run_id, operation, idempotency_key, request_digest, state,
       response_json, created_at, finished_at
FROM enno_operation_receipts_v16;

DROP TABLE enno_operation_receipts_v16;

CREATE TABLE enno_advisory_rounds_new (
...
INSERT INTO enno_advisory_rounds_new (
...
SELECT round_id, run_id, contract_revision, mutation_revision, phase, input_digest,
       policy_version, source, state, degraded, aggregate_json, created_at, updated_at
FROM enno_advisory_rounds;

CREATE TABLE enno_advisory_contributions_new (
    round_id TEXT NOT NULL REFERENCES enno_advisory_rounds_new(round_id) ON DELETE CASCADE,
...
INSERT INTO enno_advisory_contributions_new (
    round_id, slot_id, slot_rank, outcome, contribution_json, created_at
)
SELECT round_id, slot_id, slot_rank, outcome, contribution_json, created_at
FROM enno_advisory_contributions;

DROP TABLE enno_advisory_contributions;
DROP TABLE enno_advisory_rounds;

ALTER TABLE enno_advisory_rounds_new RENAME TO enno_advisory_rounds;
ALTER TABLE enno_advisory_contributions_new RENAME TO enno_advisory_contributions;
