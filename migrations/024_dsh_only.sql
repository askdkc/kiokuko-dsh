-- Remove legacy non-DSH run graphs before rebuilding the continuation state
-- around the sole authoritative DSH session identity.
CREATE TEMP TABLE __kiokuko_024_removed_runs AS
SELECT run_id FROM ledger_runs WHERE client_kind <> 'dsh';

CREATE TEMP TABLE __kiokuko_024_removed_sessions AS
SELECT session_id FROM run_intakes
WHERE run_id IN (SELECT run_id FROM __kiokuko_024_removed_runs);

DELETE FROM ledger_purge_audit
WHERE run_id IN (SELECT run_id FROM __kiokuko_024_removed_runs)
   OR event_id IN (
...
DELETE FROM akinator_sessions
WHERE id IN (SELECT session_id FROM __kiokuko_024_removed_sessions)
  AND NOT EXISTS (SELECT 1 FROM run_intakes WHERE run_intakes.session_id = akinator_sessions.id);

DROP TABLE __kiokuko_024_removed_sessions;
DROP TABLE __kiokuko_024_removed_runs;

CREATE TRIGGER ledger_runs_dsh_insert_guard
BEFORE INSERT ON ledger_runs
WHEN NEW.client_kind <> 'dsh'
BEGIN
  SELECT RAISE(ABORT, 'ledger run client_kind must be dsh');
END;

CREATE TRIGGER ledger_runs_dsh_update_guard
BEFORE UPDATE OF client_kind ON ledger_runs
WHEN NEW.client_kind <> 'dsh'
BEGIN
  SELECT RAISE(ABORT, 'ledger run client_kind must be dsh');
END;

CREATE TABLE __kiokuko_024_enno_advisory_contributions AS SELECT * FROM enno_advisory_contributions;
CREATE TABLE __kiokuko_024_enno_advisory_rounds AS SELECT * FROM enno_advisory_rounds;
CREATE TABLE __kiokuko_024_enno_dsh_continuations AS
SELECT run_id, source_session_id AS dsh_session_id, contract_revision, mutation_revision,
       attempts, directive_digest, continuation_count, total_count, updated_at
FROM enno_client_continuations WHERE client_kind = 'dsh';
CREATE TABLE __kiokuko_024_enno_execution_leases AS
SELECT run_id, contract_revision, mutation_revision, work_unit_id, route_epoch,
       owner_session_id AS dsh_session_id, lease_token_hash, lease_expires_at,
       heartbeat_at, created_at, updated_at
FROM enno_execution_leases WHERE owner_client_kind = 'dsh';
CREATE TABLE __kiokuko_024_enno_operation_receipts AS SELECT * FROM enno_operation_receipts;
CREATE TABLE __kiokuko_024_enno_resume_tokens AS
SELECT token_hash, run_id, repository_root, route_epoch,
       client_session_id AS dsh_session_id, expires_at, created_at
FROM enno_resume_tokens WHERE client_kind = 'dsh';
CREATE TABLE __kiokuko_024_enno_verifier_runs AS SELECT * FROM enno_verifier_runs;
CREATE TABLE __kiokuko_024_enno_work_units AS SELECT * FROM enno_work_units;
CREATE TABLE __kiokuko_024_enno_contracts AS
SELECT run_id, workspace, orchestration_session_id, client_session_id AS dsh_session_id,
       repository_root, task_type, status, revision, confirmation_state, attempts,
       mutation_revision, contract_json, handoff_json, intake_discovery_json,
       plan_digest, blocker, created_at, updated_at, phase, ideal_json,
       meditation_json, route_epoch
FROM enno_contracts WHERE client_kind = 'dsh';
...
INSERT INTO enno_contracts SELECT * FROM __kiokuko_024_enno_contracts;
INSERT INTO enno_work_units SELECT * FROM __kiokuko_024_enno_work_units;
INSERT INTO enno_verifier_runs SELECT * FROM __kiokuko_024_enno_verifier_runs;
INSERT INTO enno_operation_receipts SELECT * FROM __kiokuko_024_enno_operation_receipts;
INSERT INTO enno_dsh_continuations SELECT * FROM __kiokuko_024_enno_dsh_continuations;
INSERT INTO enno_resume_tokens SELECT * FROM __kiokuko_024_enno_resume_tokens;
INSERT INTO enno_execution_leases SELECT * FROM __kiokuko_024_enno_execution_leases;
INSERT INTO enno_advisory_rounds SELECT * FROM __kiokuko_024_enno_advisory_rounds;
INSERT INTO enno_advisory_contributions SELECT * FROM __kiokuko_024_enno_advisory_contributions;

DROP TABLE __kiokuko_024_enno_advisory_contributions;
DROP TABLE __kiokuko_024_enno_advisory_rounds;
DROP TABLE __kiokuko_024_enno_dsh_continuations;
DROP TABLE __kiokuko_024_enno_execution_leases;
DROP TABLE __kiokuko_024_enno_operation_receipts;
DROP TABLE __kiokuko_024_enno_resume_tokens;
DROP TABLE __kiokuko_024_enno_verifier_runs;
DROP TABLE __kiokuko_024_enno_work_units;
DROP TABLE __kiokuko_024_enno_contracts;

ALTER TABLE gateway_idempotency RENAME TO dsh_intake_idempotency;
