...
** CANCELLED consider CommonDoc from the 40ants fork
The API is Markdown. This is all that docstrings have. There is no
reason to have anything beyond what MGL-PAX::@EXTENDING-DOCUMENT
currently offers.
...
** CANCELLED C-. history depends on current package
E.g. 'clhs' works in a package where PAX is :USEd but not elsewhere.

But so does M-.'s history. This should be fixed in Slime if anywhere.
...
- State "CANCELLED" from "MAYBE" [2025-05-07 Wed 09:29] \\
  ...
  perfect way to make it work for e.g. sections. I think accessing the
...
** TODO documentation conditional on live/offline
...
Or related to [[support external links]].

This would come in handy in the SBCL documentation as well.

Or maybe this is better thought of as a fallback to an external link
if not being documented: [ironclad][asdf:system] is a broken link if
ironclad is not loaded (live doc) or not being documented (offline).
** MAYBE 40ants asdf-system-relative images?
https://github.com/40ants/doc/commit/4ae22efa20a4a2117da9dd8e96c1117458904d73
** MAYBE support generating cross-linked READMEs easily
PAX World does this.
** MAYBE Can we sometimes drop the package from a name?
The package can be considered somewhat unnecessary if a link to
SB-THREAD:JOIN-THREAD is output. On the other hand, we can't drop the
package from non-link code. Conversely, outputting JOIN-THREAD just
because the package uses SB-THREAD exposes internals.
** MAYBE smart quotes
We could convert "--" to en dash and touch up the typography in other
ways. But this would mean that a lazy docstring containing "git --
<path>" (not marked as code with backticks) would cease to be
copy-pastable. Similar problems would arise if straight double quotes
were replaced with their curly counterparts.

Maybe this could be reserved for when there is a HOME-SECTION (so
written-with-pax-in-mind can be assumed)?

Notes:
- 3bmd's smart quote escaping does't roundtrip.
- 3bmd's smart quote includes lots of stuff (e.g. arrows).
- Pandoc markdown+smart to pdf works nicely except where we output
  code as non-code (e.g. in the bullet line "- [function] ...")
** MAYBE make *document-downcase-uppercase-code* the default when HOME-SECTION?
And the output is not :PLAIN. As a side effect, this would make it
apparent that sanitization is happening.
** MAYBE make the rest of the define-concept :keys render as "See also <first key>"
** CANCELLED make "a" and "an" locative aliases of the "type"
- State "CANCELLED"  from "MAYBE"      [2026-07-02 Thu 16:29] \\
  This can be done by client code. Exporting A and AN would be a tad annoying.
so that one can write "X is a STRING" without having to disambiguate
with "X is a [STRING][class]". Even less important now that PAX links
to CLHS disambiguation pages.
** MAYBE use https://codeberg.org/dlowe/metaspectre for something?
** TODO linking in arglist default values is broken
We can't disambiguate 100% correctly: &KEY (TYPE 'FUNCTION), which can
clutter the index.

But e.g. BYTES &KEY (END (LENGTH BYTES)) shouldn't link to CL:BYTE.

Maybe on link only simple cases?

See ARGLIST-TO-TREE and ARGLIST-OBJECT-TO-MD-TREES.
* live
** MAYBE live browsing: allow filtering out local definitions when there is a clhs definition?
** MAYBE have apropos name be editable?
...
** MAYBE link asdf system to definitions in it?
...
PAX is already kind of a wiki. Live interaction is the next step,
which would move towards notebooks and GUIs.
* world
...
Documenting packages, systems like as in live browsing would be a
start.
* transcribe
...
Maybe capture output per stream?
...
** MAYBE consider font-locking codifiable and linkable stuff in docstrings
This should be doable with a lot of caching.
* literate programming
** CANCELLED reorder source definitions
May be a bit against the general source-first philosopy of PAX.
** CANCELLED local functions?
This is probably covered by NOTEs.
** CANCELLED named one-off local functions?
(defun foo (x)
  ;; Just for the name, really. Maybe control what it can close over?
  (subfoo ()
   ))

Looks like NOTE does this already.
...
* marketing
** DONE find a catchy name for the method
- Illiterate Programming: Already used in the sense of "not literate programming".
- Essay as a name: https://quotenil.com/multifaceted-development.html
  - Documentation-Driven Development: Exists, and also a misnomer.
