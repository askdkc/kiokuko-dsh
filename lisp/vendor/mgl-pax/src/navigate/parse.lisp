...
(defsection @parsing-names (:title "Parsing Names")
  ...
  "For a word, a number of @RAW-NAMEs are generated by trimming
...
(defun depluralize (string)
  ...
  (let ((l (length string))
        (r ()))
    (labels ((%desuffix (suffix)
               (let ((pos (- l (length suffix))))
                 (when (and (<= 0 pos)
                            (string-equal suffix string :start2 pos))
                   (subseq string 0 pos))))
             (desuffix (suffix)
               (when-let (x (%desuffix suffix))
                 (push x r))))
      (desuffix "ses")
      (desuffix "es")
      (desuffix "s")
  ...
  (with-type-dispatch ((string ((vector base-char) (vector character))))
    (let ((first-uppercase-pos (position-if #'upper-case-p string)))
      (when first-uppercase-pos
        (let ((last-uppercase-pos (position-if #'upper-case-p string
                                               :from-end t)))
