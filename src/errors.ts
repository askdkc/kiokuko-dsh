...
export const STORED_MEMORY_RECOVERY_MESSAGE = [
  ...
  '   macOS database path: "$HOME/Library/Application Support/kiokuko/kiokuko-dsh.sqlite3"',
  '   Linux database path: "${XDG_DATA_HOME:-$HOME/.local/share}/kiokuko/kiokuko-dsh.sqlite3"',
  "   Example (replace <new-backup.sqlite3> and choose an unused <timestamp>):",
  '   DB="$HOME/Library/Application Support/kiokuko/kiokuko-dsh.sqlite3"; for SUFFIX in "" -wal -shm -journal; do [ -e "$DB$SUFFIX" ] && mv "$DB$SUFFIX" "$DB$SUFFIX.before-restore-<timestamp>"; done; cp "<new-backup.sqlite3>" "$DB"; kiokuko setup',
...
export function databaseBackupIntegrityError(cause?: unknown): KiokukoError {
  const error = new KiokukoError(
    "INTEGRITY_ERROR",
    DATABASE_BACKUP_RECOVERY_MESSAGE,
  );
  if (cause !== undefined)
