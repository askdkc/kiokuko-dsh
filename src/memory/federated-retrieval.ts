...
export { applicabilityCompatibility } from './applicability-compatibility.js';
...
function globalLaneCandidates(
  ...
  const eligible = ranked.hits.flatMap((hit) => {
    ...
    return hasExplicitFederatedScope(entry, 'global') && autoGlobalApplicable(database, entry, fingerprint) ? [{ entry, hit }] : [];
  });
  return {
...
export async function retrieveFederatedMemory(
  ...
  const globalFingerprint = input.fingerprint ?? (project === undefined ? undefined : resolveProjectFingerprint(
    database, project, captureProjectManifestSnapshot(project), { readOnly: true }));
  const globalLane = globalEnabled
    ? globalLaneCandidates(database, input.query, Math.min(limit, policy.global.limit), runtime, globalFingerprint)
...
export async function federatedEntries(
  ...
  const globalFingerprint = input.fingerprint ?? (input.projectOnly ? undefined : resolveProjectFingerprint(
    database, input.project, captureProjectManifestSnapshot(input.project), { readOnly: true }));
  const global = input.projectOnly ? [] : globalLaneCandidates(database, input.query, Math.min(input.limit, 100), runtime, globalFingerprint).candidates.map(({ entry, hit }) => ({
    entry,
