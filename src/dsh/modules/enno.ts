...
export const ennoModule: DshModule<CoreModuleHost> = {
  ...
  async mount({ host, defer }, configuration) {
    ...
    const adapter = createDshHostAdapter(host.context, { ...config, answerReview: host.answerReviewConfig, repositoryRoot: host.repositoryRoot, runtime: host.runtime, decisions: host.decisions, semanticCompactionCoordinator: host.semanticCompaction, skillPrompts: host.prompts })
