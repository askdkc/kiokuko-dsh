...
import type { AgentReply, DeepArtifact, DeepState, GoalNode, QualityReply } from './core/contracts.js'
...
export function inputArtifacts(state: DeepState, node: GoalNode, artifacts: readonly DeepArtifact[]): DeepArtifact[] {
  if (node.quality && ['draft-a', 'draft-b'].includes(node.quality.phase) && node.quality.commonArtifactIds !== null) return currentArtifacts(state, artifacts).filter(a => node.quality!.commonArtifactIds!.includes(a.id))
...
export function evidenceReceipt(reply: AgentReply | QualityReply, candidate: GoalNode['candidate'], artifacts: readonly DeepArtifact[], inputDigest: string): NonNullable<GoalNode['receipt']> {
  const references = [...replyEvidence(reply), ...(candidate?.evidence ?? [])]
...
/** Include nested findings and assessments: untrusted references cannot hide below the top-level answer. */
export function replyEvidence(reply: AgentReply | QualityReply) {
  return [...('evidence' in reply ? reply.evidence : []),
    ...(reply.kind === 'quality-candidate' ? reply.findings.flatMap(f => f.evidence) : []),
    ...(reply.kind === 'quality-review' ? [...reply.evaluations.flatMap(e=>e.evidence), ...reply.resolutions.flatMap(r=>r.evidence)] : [])]
}
