...
export function initialDeepState(intent: DeepIntent, runId: string, context: string): DeepState {
  return DeepStateSchema.parse({ protocolVersion: intent.configuration?.reasoningMode === 'quality' ? 2 : 1, runId, startId: intent.startId, workspace: intent.workspace, sessionId: intent.sessionId, rootPath: intent.rootPath,
    ...
    nodes: [{ id: randomUUID(), parentId: null, revision: 1, question: intent.task, requirementIds: ['request'], acceptanceCriteria: [intent.task], assumptions: [], dependencies: [], depth: 0,
      replans: 0, ...(intent.configuration?.reasoningMode === 'quality' ? { quality: emptyQualityNode() } : {}), status: 'planning', activeAttemptId: null, candidate: null, proposal: null, reason: '', receipt: null }],
  })
}
