...
try {
  ...
  if (combination.includes('lisp')) {
    ...
    const discovery = await ctx.tools.execute({ callId: 'packed-typesafe-discovery', name: 'lisp_describe', arguments: {}, agent: parent, signal: new AbortController().signal })
    assert.ok(JSON.stringify(discovery).includes('kioku.typesafe:evaluate'))
