...
export async function stageModules(output) {
  ...
  for (const name of Object.keys(entries)) {
    ...
    const assetFiles = name === 'core' ? migrations : name === 'lisp' ? (await walk(join(root, 'lisp'))).map(file => `lisp/${file}`) : []
    ...
    const packageJson = { name: manifest.name, version: manifest.version, private: true, type: 'module', license: manifest.license, engines: manifest.engines,
      exports: { '.': { types: `./${entries[name].replace(/\.js$/, '.d.ts')}`, default: `./${entries[name]}` } }, files: ['dist/', 'skills/', 'scripts/', ...(name === 'core' ? ['migrations/'] : name === 'lisp' ? ['lisp/'] : []), `module-${name}.json`, 'LICENSE', 'THIRD_PARTY_NOTICES.md'], dependencies, peerDependencies,
