...
import { LayaCoreMLDecisionProvider, parseLayaHealth } from '../dist/dsh/decisions/laya-coreml.js'
...
try {
  ...
  if (values['print-config']) {
    ...
    const provider = new LayaCoreMLDecisionProvider(config['laya-coreml'])
    ...
    console.log(JSON.stringify({ live: true, readiness: readiness.state, result, capacityRejection: true, fingerprintRejection: true, legacyNoul: true,
