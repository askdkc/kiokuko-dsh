...
- `action: "review"`：採用、不適用、現在の証拠との矛盾を記録。採用と矛盾には、根拠となるソース・テスト・設定の相対パスが必須。話題が違うため不適用と判断した場合は `paths: []` を使える。現行ソースに依存する不適用判断なら、そのパスを指定する。採用では守る条件、反例、検証方法も必須。
- `action: "review_batch"`：独立した採否を最大32件、1回で記録する。1件でも無効なら全件を取り消す。未処理が複数ある場合は、`status` で確認してからまとめて送る。
...
同じrunへの再配信で対象entryのrevisionと適用モードが変わらなければ、採否を維持します。新しいentryには採否が必要です。entryの改訂または適用モードの変更では採否の世代が進み、対象全件を再確認します。配信が変わると実行証拠は失効します。検索を狭めても、既に配信した記憶への対応は消えません。申告された関連パスの内容、追加・削除、別の変更操作も確認します。`paths: []` の不適用判断はファイル変更では失効せず、entryの改訂では失効します。パスの走査は申告された対象に限定し、最大2,048項目・16 MiBです。範囲外、symlink、上限超過は検証不能になります。
