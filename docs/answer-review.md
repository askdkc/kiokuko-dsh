...
通常版とmodular coreの通常実行で、最初の回答を表示してから評価します。指摘があれば同じ主モデルが一度だけ再検討します。省略時は無効です。プラグイン設定に次を追加します。
...
評価providerは、その論理リクエストに固定された `typedDecisions` のJev／Laya／Nimbleです。別providerへの自動切替はありません。`budgetMs` は評価の上限（1〜30,000 ms）です。評価には追加推論が発生し、指摘のある場合には主モデルの追加ターンも発生します。主モデルによる再検討の実行時間は評価予算に含みません。
