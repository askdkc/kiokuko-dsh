...
モデルを選ぶと、そのまま役割の構成確認に戻ります。接続先の種類・認証・通信方式は
DSHの設定を使い、重ねて質問しません。Kiokukoは認証情報を取得・変更しません。
...
数字の自由入力も検索語として扱います。戻る・取消の後も、確定済みのモデルを保持します。

DeepとEnnoの選択肢には、macOSでは **Cmd+1〜9**、Windows/Linuxでは **Ctrl+1〜9**
を表示します。選択後、Enterで確定します。カード内では数字だけでも選べ、
10番以降は数字を続けて入力します。検索欄や予算欄に入力した数字は、そのままの値を使います。
...
プロバイダー・モデルの正確なIDをDSHに渡し、DSH本体のアダプターで設定を検証します。
テンプレートの対応付けとローカル接続の同時実行制限には、DSHの接続設定一覧を使います。
追加の接続情報が取得できないプロバイダーでも、登録済みモデルの選択後に再質問しません。

旧`modelRoutes`は、接続情報を公開しないプロバイダープラグインや旧ホスト向けの
任意設定として引き続き読み込めます。現在のDSH接続情報を優先し、実際の認証・通信方式を
上書きしません。DSH本体のモデル検証機能がない旧ホストでは、既存のAstra/Go設定に
対する互換性確認を維持します。ローカル検証の成功は、契約・認証・実サービスへの
通信成功を保証するものではありません。
