...
test('interactive setup confirms and replaces conflicts for every selected client before committing', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'kiokuko-cli-all-client-replace-'));
  const configRoot = path.join(root, 'config');
  const dataRoot = path.join(root, 'data');
  const codexConfigPath = path.join(root, '.codex', 'config.toml');
  const openCodeConfigPath = path.join(configRoot, 'opencode', 'opencode.jsonc');
  ...
  await writeFile(codexConfigPath, 'model = "keep"\n[mcp_servers.kiokuko]\ncommand = "codex-wrapper"\n');
  ...
  const output = new Writable({
    write(chunk, _encoding, callback) {
      const match = /Replace the existing (Codex|OpenCode|Claude Code|Hermes Agent) Kiokuko MCP identity/u
        .exec(chunk.toString());
      const label = match?.[1];
  ...
  assert.deepEqual(promptedClients, ['Codex', 'OpenCode', 'Claude Code', 'Hermes Agent']);
  assert.match(stdout, /Kiokuko configured for codex, opencode, claude, hermes/u);
  ...
  assert.deepEqual(claudeConfig.mcpServers.other, { type: 'http', url: 'https://example.test' });
...
test('declining a later client conflict preserves every earlier approved client file', async () => {
  ...
  const codexConfigPath = path.join(root, '.codex', 'config.toml');
  const openCodeConfigPath = path.join(configRoot, 'opencode', 'opencode.jsonc');
  const codexOriginal = 'model = "keep"\n[mcp_servers.kiokuko]\ncommand = "codex-wrapper"\n';
  const openCodeOriginal = '{"theme":"keep","mcp":{"kiokuko":{"type":"remote"}}}\n';
  await mkdir(path.dirname(codexConfigPath), { recursive: true });
  await mkdir(path.dirname(openCodeConfigPath), { recursive: true });
  ...
  await assert.rejects(access(path.join(dataRoot, 'kiokuko', 'kiokuko.sqlite3')));
});
