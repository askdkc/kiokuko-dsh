...
test("wraps unexpected liveness failures as a fixed service-unavailable error", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      assert.equal(
        error.message,
        "Unable to determine server process liveness",
      );
      assert.deepEqual(error.details, {});
      assert.equal(
        error.message.includes("raw liveness failure detail"),
        false,
      );
...
test("propagates the typed security error for a symlinked descriptor", async () => {
  ...
  await assert.rejects(
    () => getServerStatus({ descriptorPath }),
    (error: unknown) =>
      error instanceof KiokukoError && error.code === "SECURITY_REJECTION",
  );
});
