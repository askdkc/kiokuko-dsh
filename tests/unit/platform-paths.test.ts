...
test("derives a per-database lock path from the resolved database path", () => {
  const databasePath = "/tmp/kiokuko-relative/../kiokuko-dsh.sqlite3";
  const fingerprint = createHash("sha256")
    .update("/tmp/kiokuko-dsh.sqlite3")
    .digest("hex");
  assert.equal(
...
test("uses XDG data home on Linux", () => {
  assert.equal(
    ...
    "/tmp/xdg-data/kiokuko/kiokuko-dsh.sqlite3",
  );
});
...
test("uses an explicit isolated Kiokuko data directory for database and runtime state", () => {
  ...
  assert.equal(
    getGlobalDatabasePath(options),
    "/work/kiokuko/.kiokuko-dev/kiokuko-dsh.sqlite3",
  );
...
test("an explicit Kiokuko data directory overrides Linux XDG data and runtime directories", () => {
  ...
  assert.equal(
    getGlobalDatabasePath(options),
    "/work/kiokuko-data/kiokuko-dsh.sqlite3",
  );
  ...
  for (const configured of [
    ...
    assert.throws(
      () =>
        getGlobalDatabasePath({
          platform: "darwin",
          env: { HOME: "/Users/test", KIOKUKO_DATA_DIR: configured },
        }),
      (error: unknown) => {
...
test("falls back to the platform home data directory", () => {
  assert.equal(
    ...
    "/tmp/home/.local/share/kiokuko/kiokuko-dsh.sqlite3",
  );
  assert.equal(
    getGlobalDatabasePath({
      platform: "darwin",
    ...
    "/tmp/home/Library/Application Support/kiokuko/kiokuko-dsh.sqlite3",
  );
  assert.equal(
    getGlobalDatabasePath({
      platform: "win32",
    ...
    String.raw`C:\Users\test\AppData\Local\kiokuko\kiokuko-dsh.sqlite3`,
  );
});
...
test("derives documented global Codex, OpenCode, and Claude paths without touching the real home directory", () => {
  ...
  assert.equal(
    getCodexConfigPath(options),
    "/tmp/fake-home/.codex/config.toml",
  );
  assert.equal(getCodexHooksPath(options), "/tmp/fake-home/.codex/hooks.json");
  assert.equal(
    getCodexInstructionsPath(options),
    "/tmp/fake-home/.codex/AGENTS.md",
  );
  assert.equal(
    getCodexSkillsDirectory(options),
    "/tmp/fake-home/.agents/skills",
  );
  assert.equal(
    getOpenCodeConfigDirectory(options),
    "/tmp/fake-config/opencode",
  );
  assert.equal(
    getOpenCodeInstructionsPath(options),
    "/tmp/fake-config/opencode/AGENTS.md",
  );
  assert.equal(
    getOpenCodeSkillsDirectory(options),
    "/tmp/fake-config/opencode/skills",
  );
  assert.equal(
    getOpenCodeEnnoPluginPath(options),
    "/tmp/fake-config/opencode/plugins/kiokuko-enno-oduno.js",
  );
  ...
  assert.equal(
    getClaudeInstructionsPath(options),
    "/tmp/fake-home/.claude/CLAUDE.md",
  );
  assert.equal(
    getClaudeSkillsDirectory(options),
    "/tmp/fake-home/.claude/skills",
  );
  assert.equal(
    getLegacyClaudePromptHookSettingsPath(options),
    "/tmp/fake-home/.claude/settings.json",
  );
  assert.equal(
    getClaudeSettingsPath(options),
    "/tmp/fake-home/.claude/settings.json",
  );
  assert.equal(
    getLegacyOpenCodeLoopGuardPath(options),
    "/tmp/fake-config/opencode/plugins/kiokuko-loop-guard.js",
  );
  assert.equal(
    getCodexConfigPath({
      ...options,
      env: { ...options.env, CODEX_HOME: "/tmp/custom-codex" },
    }),
  ...
  assert.equal(
    getClaudeMcpConfigPath({
      ...options,
      env: { ...options.env, CLAUDE_CONFIG_DIR: "/tmp/custom-claude" },
    }),
...
test("derives native standard-skill directories on macOS, Linux, and Windows", async () => {
  assert.equal(
    getCodexSkillsDirectory({
      platform: "darwin",
      env: { HOME: "/Users/test", CODEX_HOME: "/custom/codex" },
    }),
  ...
  assert.equal(
    await getHermesSkillsDirectory({
      platform: "darwin",
      env: { HERMES_HOME: "/Users/test/.hermes/profiles/work" },
    }),
    "/Users/test/.hermes/profiles/work/skills",
  );
  ...
  assert.equal(
    await getHermesSkillsDirectory({
      platform: "linux",
      env: { HERMES_HOME: "/home/test/.hermes/profiles/work" },
    }),
    "/home/test/.hermes/profiles/work/skills",
  );
  ...
  assert.equal(
    getCodexSkillsDirectory({ platform: "win32", env: windowsEnvironment }),
    String.raw`C:\Users\test\.agents\skills`,
  );
  assert.equal(
    getOpenCodeConfigDirectory({ platform: "win32", env: windowsEnvironment }),
    String.raw`C:\Users\test\.config\opencode`,
  );
  assert.equal(
    getOpenCodeInstructionsPath({ platform: "win32", env: windowsEnvironment }),
    String.raw`C:\Users\test\.config\opencode\AGENTS.md`,
  );
  assert.equal(
    getOpenCodeSkillsDirectory({ platform: "win32", env: windowsEnvironment }),
    String.raw`C:\Users\test\.config\opencode\skills`,
  );
  assert.equal(
    getClaudeSkillsDirectory({ platform: "win32", env: windowsEnvironment }),
    String.raw`D:\Claude\skills`,
  );
  assert.equal(
    getLegacyClaudePromptHookSettingsPath({
      platform: "win32",
    ...
    String.raw`D:\Claude\settings.json`,
  );
  assert.equal(
    getLegacyOpenCodeLoopGuardPath({
      platform: "win32",
    ...
    String.raw`C:\Users\test\.config\opencode\plugins\kiokuko-loop-guard.js`,
  );
  assert.equal(
    await getHermesSkillsDirectory({
      platform: "win32",
    ...
    String.raw`D:\Hermes\profiles\work\skills`,
  );
});
...
test("resolves Linux and macOS Hermes default, named, custom, and profile-shaped homes", async () => {
  for (const platform of ["linux", "darwin"] as const) {
    const root = await mkdtemp(
      path.join(tmpdir(), `kiokuko-hermes-paths-${platform}-`),
    );
    ...
    assert.equal(
      await getHermesConfigPath(environment),
      path.join(namedProfile, "config.yaml"),
    );
    assert.equal(
      await getHermesSkillsDirectory(environment),
      path.join(namedProfile, "skills"),
    );
    ...
    assert.equal(
      await getHermesHome({
        platform,
        env: { HOME: home, HERMES_HOME: customRoot },
      }),
      customRoot,
    );
    ...
    assert.equal(
      await getHermesHome({
        platform,
        env: { HOME: home, HERMES_HOME: customProfile },
      }),
      customProfile,
    );
  }
...
test("rejects invalid or missing active Hermes profiles on Linux and macOS", async () => {
  for (const platform of ["linux", "darwin"] as const) {
    for (const marker of [
      ...
      const root = await mkdtemp(
        path.join(tmpdir(), `kiokuko-hermes-invalid-${platform}-`),
      );
      const home = path.join(root, "home");
      const hermesRoot = path.join(home, ".hermes");
...
test("uses hermes config path output when no active_profile marker exists", async () => {
  const root = await mkdtemp(
    path.join(tmpdir(), "kiokuko-hermes-config-path-"),
  );
  const home = path.join(root, "home");
...
test("rejects a Hermes CLI path that disagrees with a sticky active profile", async () => {
  const root = await mkdtemp(
    path.join(tmpdir(), "kiokuko-hermes-sticky-disagreement-"),
  );
  const home = path.join(root, "home");
  const hermesRoot = path.join(home, ".hermes");
  ...
  await assert.rejects(
    getHermesHome({
      ...
      env: {
        HOME: home,
        PATH: bin,
        HERMES_CONFIG_PATH: path.join(otherHome, "config.yaml"),
      },
    }),
    (error: unknown) =>
...
test("Hermes config path command failures and malformed output fail explicitly", async () => {
  for (const [name, script] of [
    ...
    const root = await mkdtemp(
      path.join(tmpdir(), `kiokuko-hermes-cli-${name}-`),
    );
    const home = path.join(root, "home");
    const bin = path.join(root, "bin");
