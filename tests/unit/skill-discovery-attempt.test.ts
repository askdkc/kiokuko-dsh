...
} from "../../src/akinator/skill-discovery-attempt.js";
...
function summary(
  queries: string[],
  selectedSkills: SkillDiscoverySummary["selected"] = [],
...
test("discovery attempts distinguish digest replay, reserve active work, and consume a run-wide budget", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      completeAgentTaskSkillDiscoveryAttempt(database, first, firstSummary),
      firstSummary,
    );
    assert.deepEqual(claimAgentTaskSkillDiscoveryAttempt(database, first), {
      kind: "replay",
    ...
    assert.deepEqual(readAgentTaskSkillDiscoveryAttempt(database, first), {
      kind: "replay",
      summary: firstSummary,
    });
    ...
    assert.throws(
      () =>
        claimAgentTaskSkillDiscoveryAttempt(
          database,
          identity("c".repeat(64), "zenki"),
        ),
      /already in progress/iu,
    );
    assert.throws(
      () => claimAgentTaskSkillDiscoveryAttempt(database, second),
      /already in progress/iu,
    );
    ...
    assert.deepEqual(
      completeAgentTaskSkillDiscoveryAttempt(database, second, secondSummary),
      secondSummary,
    );
    ...
    const empty = summary([]);
    assert.deepEqual(
      completeAgentTaskSkillDiscoveryAttempt(database, exhausted, empty),
      empty,
    );
    assert.deepEqual(readAgentTaskSkillDiscoveryAttempt(database, exhausted), {
      kind: "replay",
      summary: empty,
    });
    assert.deepEqual(
...
test("failed discovery consumes its reserved budget while a changed digest can use only what remains", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      claimAgentTaskSkillDiscoveryAttempt(database, failed, {
        queryBudget: 1,
        selectionBudget: 1,
      }),
      {
    ...
    assert.throws(
      () =>
        failAgentTaskSkillDiscoveryAttempt(
          database,
          failed,
          new KiokukoError("CONFLICT", "provider failed"),
        ),
...
test("generic attempt replay preserves typed provider failures and rejects unknown stored codes", async () => {
  const database = await temporaryDatabase(
    "discovery-provider-failure-boundary",
  );
  try {
    const malformed = identity("f".repeat(64), "zenki");
    claimAgentTaskSkillDiscoveryAttempt(database, malformed, {
      queryBudget: 1,
      selectionBudget: 1,
    });
    assert.throws(
      () =>
        failAgentTaskSkillDiscoveryAttempt(
          database,
          malformed,
          new SkillProviderError("registry_invalid_response"),
        ),
      (error: unknown) =>
        error instanceof SkillProviderError &&
