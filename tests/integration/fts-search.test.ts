...
test("searches Japanese and English content, tags, filters, and excludes superseded entries", async () => {
  ...
  try {
    ...
    const japaneseResults = searchEntries(database, {
      workspace: "project:search",
      query: "日本語",
    });
    assert.ok(japaneseResults.items.some((item) => item.id === japanese.id));
    assert.deepEqual(
      searchEntries(database, {
        workspace: "project:search",
        query: "日本語",
      }).items.map((item) => item.id),
      japaneseResults.items.map((item) => item.id),
    );

    const tagged = searchEntries(database, {
      workspace: "project:search",
      query: "memory",
      tag: "english",
    });
    assert.deepEqual(
      tagged.items.map((item) => item.id),
      [english.id],
    );
    const verified = searchEntries(database, {
      workspace: "project:search",
      query: "memory",
      status: "verified",
    });
    assert.deepEqual(
      verified.items.map((item) => item.id),
      [japanese.id],
    );
    const all = searchEntries(database, {
      workspace: "project:search",
      query: "memory",
    });
    assert.ok(all.items.some((item) => item.id === replacement.id));
    assert.equal(
      all.items.some((item) => item.id === old.id),
      false,
    );
  } finally {
    database.close();
...
test("literal punctuation and empty search are safe no-result operations", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      searchEntries(database, { workspace: "project:literal", query: "   " })
        .items,
      [],
    );
  } finally {
...
test("recall respects limit and character budget and marks stored data as untrusted", async () => {
  ...
  try {
    ...
    const result = recallEntries(database, {
      workspace: "project:budget",
      query: "budget",
      limit: 1,
      maxChars: 80,
    ...
    assert.equal(
      readEntry(database, { workspace: "project:budget", entryId: recorded.id })
        .body.length > 80,
      true,
    );
  } finally {
