...
test("starts a trimmed task through the shared domain and store service", async () => {
  ...
  try {
    ...
    assert.equal(
      database
        .prepare("SELECT COUNT(*) AS count FROM akinator_sessions")
  ...
  try {
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.equal(
          JSON.stringify(typed.details ?? {}).includes(secret),
          false,
        );
        return true;
      },
  ...
  try {
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.equal(
          typed.message,
          "Akinator task snapshot exceeds 65536 bytes",
        );
        return true;
      },
    );
  ...
  try {
    ...
    assert.equal(
      database
        ...
        .get<{ count: number }>(started.session.id)?.count,
      1,
    );
  } finally {
    database.close();
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.equal(
          typed.message,
          "sanitized answer snapshot exceeds 65536 bytes",
        );
        return true;
      },
    );
...
test("conflicts on a different answer for an already-answered question without mutating it", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        (error as { code?: string; message?: string }).code === "CONFLICT" &&
        (error as { message?: string }).message ===
          "Akinator answer conflicts with the existing answer",
    );
    const session = database
      .prepare(
        "SELECT question_count AS questionCount FROM akinator_sessions WHERE id = ?",
      )
      .get<{ questionCount: number }>(started.session.id);
...
test("returns the exact empty context shape while intake still needs an answer", async () => {
  const database = await temporaryDatabase(
    "akinator-service-context-needs-answer",
  );
  try {
    const started = await startAkinatorService(database, {
...
test("returns empty stored context when no local entry is available", async () => {
  const database = await temporaryDatabase(
    "akinator-service-no-skill-fallback",
  );
  try {
    const started = await startAkinatorService(database, {
...
test("keeps the orchestrator limited to intake mutation without a task-memory compatibility facade", () => {
  const source = readFileSync(
    new URL("../../src/akinator/orchestrator.ts", import.meta.url),
    "utf8",
  );
  assert.doesNotMatch(
    source,
    /database\.prepare|TASK_TYPES|inferTaskType|nextQuestion|recommendedTags|searchEntries|getAkinatorContextService|withImmediateTransaction|Akinator is waiting/u,
  );
...
test("removes the fixed-source compatibility adapter", () => {
  assert.equal(
    existsSync(new URL("../../src/knowledge/sources.ts", import.meta.url)),
    false,
  );
});
