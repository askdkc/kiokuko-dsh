...
async function temporaryDatabase(prefix: string) {
  const directory = await mkdtemp(
    path.join(tmpdir(), `kiokuko-doctor-${prefix}-`),
  );
...
async function invokeDoctor(
  ...
  stdout: string;
  prompt: string;
  response?: { data: Record<string, unknown>; ok: boolean };
}> {
  let stdout = "";
...
test("missing-location cleanup removes only absent location rows and preserves repositories", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      candidates.map((location) => location.canonicalRoot),
      [missingRoot],
    );

    const removed = removeMissingRepositoryLocations(
      value.database,
      candidates,
    );
...
test("missing-location cleanup rechecks a root before deleting its registry row", async () => {
  ...
  try {
    ...
    await mkdir(root);
    assert.equal(
      removeMissingRepositoryLocations(value.database, candidates),
      0,
    );
    assert.equal(locationCount(value.database), 1);
  } finally {
...
test("JSON doctor never prompts or cleans missing locations", async () => {
  ...
  assert.equal(
    (result.response?.data.checks as { bindings: { ok: boolean } }).bindings.ok,
    false,
  );
...
test("doctor reports a forced sqlite-vec backend that cannot be loaded", async () => {
  ...
  try {
    const config = requireEnabledEmbeddingConfig(
      parseEmbeddingConfig(embeddingEnvironment),
    );
  ...
  const result = await runDoctor(
    {
      databasePath: value.databasePath,
      runtimeDescriptorPath: path.join(
        value.directory,
        "runtime",
        "server.json",
      ),
...
test("doctor reports an unmanaged Codex Kiokuko MCP identity", async () => {
  ...
  const result = await withDoctorEnvironment(value.directory, codexHome, () =>
    runDoctor(),
  );
