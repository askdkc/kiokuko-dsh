...
function memoryPolicy(
  availability: "available" | "missing" | "unknown" | null,
) {
...
async function fixture(suffix: string) {
  const directory = await mkdtemp(
    path.join(tmpdir(), `kiokuko-agent-capability-gate-${suffix}-`),
  );
...
async function makePriorContextActionable(
  ...
  if (
    opened.nextAction === "required_capability_unavailable" ||
    opened.capabilities.recommendations.some(
  ...
  return (await request(baseUrl, "/api/v1/agent/runs", { key: openKey, body }))
    .data;
}
...
test("generic Agent API withholds actionable repair memory but continues when memory-reasoning is missing", async () => {
  ...
  try {
    ...
    const stopped = await makePriorContextActionable(
      value.runtime.url,
      "missing-open",
      body,
      first.data,
    ...
    assert.ok(
      stopped.capabilities.recommendations.some(
        (item: any) =>
...
test("generic Agent API proceeds with actionable repair memory only for the exact available local skill", async () => {
  ...
  try {
    const body = openBody(
      value.workspace,
      soulCapabilities({ kind: "skill", name: "memory-reasoning" }),
    );
    const first = await request(value.runtime.url, "/api/v1/agent/runs", {
      key: "available-open",
      body,
    ...
    assert.equal(first.response.status, 200);
    const proceeded = await makePriorContextActionable(
      value.runtime.url,
      "available-open",
      body,
      first.data,
    ...
    assert.ok(
      proceeded.capabilities.recommendations.some(
        (item: any) =>
          item.name === "memory-reasoning" &&
          item.required === true &&
          item.availability === "available",
      ),
      JSON.stringify(proceeded),
    );
  } finally {
    await value.runtime.close();
  }
...
test("exact open replay keeps one mutation acknowledgement but re-evaluates current capability-gated context", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-agent-capability-replay-"),
  );
  const databasePath = path.join(directory, "kiokuko-dsh.sqlite3");
  ...
  try {
    const first = await request(runtime.url, "/api/v1/agent/runs", {
      key: "capability-replay-open",
      body,
    });
    assert.equal(first.response.status, 200);
    ...
    assert.equal(
      first.data.context.items[0]?.selectionReasons.includes(
    ...
    try {
      ...
      const storedAcknowledgement = JSON.parse(response) as Record<
        string,
        unknown
      >;
    ...
    const replay = await request(runtime.url, "/api/v1/agent/runs", {
      key: "capability-replay-open",
      body,
    });
    ...
    try {
      ...
      assert.equal(
        after
          .prepare(
            `SELECT COUNT(*) AS count FROM gateway_idempotency WHERE scope = 'agent.run.open'`,
          )
          .get<{ count: number }>()?.count,
        1,
      );
      assert.equal(
        after
          .prepare(
            `SELECT response_json AS response FROM gateway_idempotency WHERE scope = 'agent.run.open'`,
          )
          .get<{ response: string }>()?.response,
  ...
  try {
    ...
    try {
      recordEntry(newlyRelevant, {
        ...
        kind: "lesson",
        title:
          "Beacon implementation regression workflow current profile guidance",
    ...
    const revised = await request(
      value.runtime.url,
      `/api/v1/agent/runs/${first.data.runId}/checkpoints`,
      {
    ...
    const deliveriesBeforeReplay = database
      .prepare(
        "SELECT COUNT(*) AS count FROM context_deliveries WHERE run_id = ?",
      )
      .get<{ count: number }>(first.data.runId)?.count;
    ...
    assert.equal(replay.data.context, null);
    assert.ok(
      replay.data.capabilities.recommendations.some(
        (item: any) =>
          item.name === "memory-reasoning" &&
          item.required === true &&
          item.availability === "missing",
...
test("exact open replay rejects a terminal authoritative run without another delivery", async () => {
  ...
  try {
    ...
    assert.equal(first.response.status, 200);
    const database = openConnection(value.databasePath);
    const deliveriesBefore = database
      .prepare(
        "SELECT COUNT(*) AS count FROM context_deliveries WHERE run_id = ?",
      )
      .get<{ count: number }>(first.data.runId)?.count;
    ...
    const closed = await request(
      value.runtime.url,
      `/api/v1/agent/runs/${first.data.runId}/close`,
      {
    ...
    try {
      assert.equal(
        verified
          ...
          .get<{ status: string }>(first.data.runId)?.status,
        "completed",
      );
      assert.equal(
        verified
...
test("generic Agent build does not treat managed external skill references as ordinary memory", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-agent-external-reference-"),
  );
  ...
  try {
    ...
    assert.ok(
      opened.data.context.items.every(
        (item: any) => item.origin === "ecosystem",
      ),
    );
    assert.equal(
      opened.data.capabilities.recommendations.some(
        (item: any) => item.name === "memory-reasoning",
      ),
      false,
    );
  } finally {
...
test("generic Agent open and answer apply identical missing, unknown, and available capability gates", async () => {
  ...
  const scenarios = [
    ...
    {
      label: "unknown",
      catalog: soulCapabilities({ kind: "invalid", name: "invalid" }),
  ...
  try {
    for (const scenario of scenarios) {
      ...
      assert.deepEqual(
        ready.data.memoryPolicy,
        memoryPolicy(scenario.availability),
      );
      ...
      const answerBody = {
        ...
        questionId: "target",
        value: "src/beacon.ts",
        ...(scenario.catalog === undefined
          ? {}
          : { capabilities: scenario.catalog }),
      };
      const answered = await request(
        value.runtime.url,
        `/api/v1/agent/runs/${opened.data.runId}/intake/answers`,
        {
      ...
      assert.deepEqual(
        answered.data.memoryPolicy,
        memoryPolicy(scenario.availability),
      );

      for (const response of [ready.data, answered.data]) {
        assert.ok(
          response.capabilities.recommendations.some(
            (item: any) =>
              item.name === "memory-reasoning" &&
              item.required === true &&
        ...
        enqueueWrite: async (operation: () => unknown) => {
          ...
          try {
            ...
            const brokerDatabase = (
              this as unknown as { database: ReturnType<typeof openConnection> }
            ).database;
            const selected = brokerDatabase
              .prepare(
                "SELECT id FROM entries WHERE workspace = ? ORDER BY id LIMIT 1",
              )
              .get<{ id: string }>(value.workspace);
            assert.ok(selected);
            const entry = readEntry(brokerDatabase, {
              workspace: value.workspace,
              entryId: selected.id,
            });
