...
async function migrationFixture(): Promise<{
  ...
  const directory = await mkdtemp(path.join(tmpdir(), 'kiokuko-cjk-migration-'));
  ...
  for (const name of names) {
    ...
    await copyFile(path.join(migrationsSource, name), path.join(migrationsDirectory, name));
  }
  return { databasePath: path.join(directory, 'kiokuko.sqlite3'), migrationsDirectory };
}
...
function seedJapaneseEntry(database: ReturnType<typeof openConnection>): void {
  ...
  `).run();
  const rowid = database.prepare("SELECT rowid FROM entries WHERE id = 'entry-cjk-upgrade'")
    .get<{ rowid: number }>()?.rowid;
  assert.ok(rowid);
  const projection = [rowid, revision.title, revision.body, '', '履歴保全'] as const;
  database.prepare('INSERT INTO entries_fts(rowid, title, body, summary, tags_text) VALUES (?, ?, ?, ?, ?)')
    .run(...projection);
  database.prepare('INSERT INTO entries_trigram(rowid, title, body, summary, tags_text) VALUES (?, ?, ?, ?, ?)')
    .run(...projection);
  database.prepare(`
...
test('migration 020 preserves current rows and rebuilds separate external-content word and trigram indexes', async () => {
  ...
  try {
    ...
    assert.equal(database.prepare('SELECT COUNT(*) AS count FROM entries_fts WHERE entries_fts MATCH ?')
      .get<{ count: number }>('"マイグレーション"')?.count, 0);
...
test('the bundled migration 020 rollback restores both legacy indexes and the schema marker', async () => {
  ...
  try {
    ...
    assert.deepEqual(migrateDatabase(database, fixture.migrationsDirectory).applied, [20]);
    const down = await readFile(path.join(migrationsSource, 'down', '020_cjk_fts.sql'), 'utf8');
    database.exec(down);

    assert.equal(database.prepare('SELECT MAX(version) AS version FROM schema_migrations').get<{ version: number }>()?.version, 19);
    const wordDefinition = database.prepare("SELECT sql FROM sqlite_master WHERE name = 'entries_fts'")
      .get<{ sql: string }>()?.sql ?? '';
    const trigramDefinition = database.prepare("SELECT sql FROM sqlite_master WHERE name = 'entries_trigram'")
      .get<{ sql: string }>()?.sql ?? '';
    assert.match(wordDefinition, /unicode61 remove_diacritics 2/u);
    assert.match(trigramDefinition, /tokenize='trigram'/u);
    assert.equal(database.prepare('SELECT COUNT(*) AS count FROM entries_fts').get<{ count: number }>()?.count, 1);
    assert.equal(database.prepare('SELECT COUNT(*) AS count FROM entries_trigram').get<{ count: number }>()?.count, 1);
    assert.equal(database.prepare("SELECT 1 AS present FROM sqlite_master WHERE name = 'entry_search_documents'").get(), undefined);
  } finally {
    database.close();
  }
});
