...
async function session(baseUrl: string): Promise<string> {
  ...
  if (!cookie?.startsWith("kiokuko_ui_session="))
    throw new Error("UI session cookie was not issued");
  return cookie;
}
...
async function fixture() {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-web-memory-revisions-"),
  );
...
test("Web memory uses only the current immutable revision for editing, filters, tags, and search", async () => {
  ...
  const web = await startWebServer({
    ...
    port: 0,
    httpOptions: {
      runtimeDirectory: path.join(path.dirname(data.databasePath), "runtime"),
    },
  });
  try {
    ...
    assert.deepEqual(
      ...
      ["project:web-revisions"],
    );

    const detail = (await webFetch(
      web.url,
      `/api/entries/${data.entry.id}?workspace=project%3Aweb-revisions`,
    ).then((response) => response.json())) as {
      entry: { revision: number; title: string };
    };
    assert.deepEqual(
      { revision: detail.entry.revision, title: detail.entry.title },
      { revision: 1, title: "PostgreSQL" },
    );

    const firstSearch = (await webFetch(
      web.url,
      "/api/entries?workspace=project%3Aweb-revisions&q=PGroonga",
    ).then((response) => response.json())) as {
      entries: Array<{ id: string }>;
    };
    assert.deepEqual(
      firstSearch.entries.map((item) => item.id),
      [data.entry.id],
    );

    const update = await webFetch(
      web.url,
      `/api/entries/${data.entry.id}?workspace=project%3Aweb-revisions`,
      {
        method: "PUT",
    ...
    const editedDetail = (await webFetch(
      web.url,
      `/api/entries/${data.entry.id}?workspace=project%3Aweb-revisions`,
    ).then((response) => response.json())) as {
    ...
    assert.deepEqual(
      { revision: editedDetail.entry.revision, body: editedDetail.entry.body },
      { revision: 2, body: "SQLite FTS5 trigram" },
    );
    ...
    for (const [query, expected] of [
      ...
      const result = (await webFetch(
        web.url,
        `/api/entries?workspace=project%3Aweb-revisions&q=${query}`,
      ).then((response) => response.json())) as { entries: unknown[] };
    ...
    for (const [parameter, expected] of [
      ...
      const result = (await webFetch(
        web.url,
        `/api/entries?workspace=project%3Aweb-revisions&${parameter}`,
      ).then((response) => response.json())) as { entries: unknown[] };
      assert.equal(result.entries.length, expected, `filter ${parameter}`);
    }
    const tags = (await webFetch(
      web.url,
      "/api/tags?workspace=project%3Aweb-revisions",
    ).then((response) => response.json())) as {
      tags: Array<{ tag: string; count: number }>;
    };
    assert.deepEqual(tags.tags, [{ tag: "sqlite", count: 1 }]);

    const stale = await webFetch(
      web.url,
      `/api/entries/${data.entry.id}?workspace=project%3Aweb-revisions`,
      {
        method: "PUT",
  ...
  try {
    assert.deepEqual(
      database
        ...
        .all<{ revision: number }>(data.entry.id)
...
test("Web JSON mutations fail closed on bytes, syntax, media type, and closed body schemas", async () => {
  ...
  const cases: Array<{ name: string; contentType?: string; body?: BodyInit }> =
    [
      ...
      {
        name: "duplicate expectedRevision",
        contentType: "application/json",
        body: '{"expectedRevision":999,"expectedRevision":1,"kind":"lesson","title":"forged revision","body":"must not persist","summary":null,"scope":{},"provenance":{},"tags":[]}',
      },
      {
        name: "duplicate nested scope key",
        contentType: "application/json",
        body: '{"expectedRevision":1,"kind":"lesson","title":"forged scope","body":"must not persist","summary":null,"scope":{"identity":"first","identity":"last"},"provenance":{},"tags":[]}',
      },
      {
        name: "non-finite nested number",
        contentType: "application/json",
        body: '{"expectedRevision":1,"kind":"lesson","title":"non-finite","body":"must not persist","summary":null,"scope":{"weight":1e400},"provenance":{},"tags":[]}',
      },
      {
        name: "unknown entry-update field",
        contentType: "application/json",
        body: validUpdate.slice(0, -1) + ',"unexpected":true}',
      },
      { name: "array body", contentType: "application/json", body: "[]" },
      { name: "empty body", contentType: "application/json" },
      {
        name: "absent media type",
        body: Uint8Array.from(Buffer.from(validUpdate)),
      },
      {
  ...
  try {
    for (const invalid of cases) {
      ...
      if (invalid.contentType !== undefined)
        headers.set("content-type", invalid.contentType);
      ...
      const detail = (await fetch(`${web.url}${target}`, {
        headers: { cookie },
    ...
    const unknownCuratorField = await fetch(
      `${web.url}/api/curator/globalize`,
      {
        method: "POST",
  ...
  try {
    ...
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM entries WHERE workspace = 'global'",
        )
...
test("Web JSON parsing does not misclassify unexpected parser failures as request validation", async () => {
  ...
  try {
    ...
    const response = await fetch(
      `${web.url}/api/entries/${data.entry.id}?workspace=project%3Aweb-revisions`,
      {
        method: "PUT",
        headers: { cookie, "content-type": "application/json" },
        body,
    ...
    const cursor = Buffer.from(
      '[1,0,null,"source","slug","provider","skill"]',
      "utf8",
    ).toString("base64url");
    const cursorResponse = await fetch(
      `${web.url}/api/skills?cursor=${cursor}`,
      { headers: { cookie } },
    );
