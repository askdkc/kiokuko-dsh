...
} from "../../src/enno-oduno/adapters.js";
...
const capabilities = [
  {
    kind: "skill",
    name: "kiokuko-soul",
    description: "Routes work to every applicable Kiokuko Skill.",
  },
  {
    kind: "skill",
  ...
  },
  {
    ...
    description: "UI interaction and accessibility guidance.",
  },
];

function advisoryContributions(phase: AdvisoryPhase) {
  return ADVISORY_SLOT_DEFINITIONS.filter((slot) => slot.phase === phase).map(
    (slot) => ({
      slotId: slot.slotId,
...
function advisoryDigest(
  phase: AdvisoryPhase,
  contractRevision: number,
  mutationRevision: number,
  context: AdvisoryContext,
): string {
  return advisoryInputDigest({
    phase,
    contractRevision,
    mutationRevision,
...
function executionCredentials(response: {
  executionLease?:
    { leaseToken: string; routeEpoch: number } | null | undefined;
}) {
...
async function fixture() {
  ...
  const databaseDirectory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-enno-db-"),
  );
  const databasePath = path.join(databaseDirectory, "kiokuko-dsh.sqlite3");
...
function submitPreparedIdeal(
  ...
  return submitOdunoIdeal(database, {
    ...
    ideal: {
      ...
      principles: [
        "Realize the task handoff while preserving every explicit constraint",
      ],
      ...
      successSignals: [
        prepared.intake.profile.expected ??
...
function submitMeditation(
  ...
  return submitOdunoMeditation(database, {
    ...
    meditation: {
      summary:
        "Inspected the realized ideal for obsolete tests and functions without mutating the repository",
...
async function plannedExecution(
  ...
  const idealized = submitPreparedIdeal(
    database,
    prepared,
    `ideal-${requestId}`,
  );
  ...
  const response = await submitEnnoPlan(database, {
      ...
      units: [
        {
          ...
          focusedVerifiers: (options.focusedVerifiers ?? []).map((item) => ({
            ...item,
            cwd: ".",
          })),
        },
    ...
    skillRequirements: [],
    finalVerifiers: [{ ...finalVerifier, cwd: "." }],
...
test("task_prepare derives the Oduno ideal before handing the request to harness-specific Zenki", async () => {
  ...
  try {
    ...
    assert.equal(
      planned.prepared.ennoOduno.orchestrationId,
      planned.prepared.intake.sessionId,
    );
    ...
    assert.equal(
      planned.prepared.ennoOduno.directive?.handoff?.sourceRole,
      "enno-oduno",
    );
    assert.equal(
      planned.prepared.ennoOduno.directive?.handoff?.taskType,
      "debug",
    );
    assert.match(
      planned.prepared.ennoOduno.directive?.handoff?.objective ?? "",
      /src\/add\.js/u,
    );
    assert.match(
      planned.prepared.ennoOduno.directive?.handoff?.objective ?? "",
    ...
    assert.equal(
      planned.prepared.ennoOduno.directive?.harness.kind,
      "opencode",
    );
    assert.equal(
      planned.prepared.ennoOduno.directive?.harness.continuation,
      "session_idle_plugin",
    );
    assert.equal(planned.prepared.ennoOduno.nextAction, "submit_ideal");
    assert.match(
      planned.prepared.ennoOduno.directive?.objective ?? "",
      /optimal goal/iu,
    );
    ...
    assert.match(
      planned.idealized.ennoOduno.directive?.objective ?? "",
      /one cohesive externally observable function or use-case contract/iu,
    );
    assert.match(
      planned.idealized.ennoOduno.directive?.objective ?? "",
      /focused runnable test target/iu,
    );
    ...
    assert.deepEqual(claimed.directive?.requiredSkills, [
      "kiokuko-soul",
      "kiokuko-single-purpose-functions",
    ]);
    assert.deepEqual(claimed.directive?.workUnit?.skillNames, [
      "kiokuko-soul",
      "kiokuko-single-purpose-functions",
    ]);
    ...
    `,
      ...
      .get<{ clientKind: string; clientSessionId: string }>(
    ...
    `,
        ...
        .get<{ count: number }>(planned.identity.runId)?.count,
      1,
    );

    assert.throws(
      () =>
        decideAdapterContinuation(database, "opencode", {
    ...
    `,
        ...
        .get<{ clientSessionId: string }>(planned.identity.runId)
        ?.clientSessionId,
      planned.hostSessionId,
    );
    assert.equal(
      database
        .prepare(
    ...
    `,
        ...
        .get<{ count: number }>(planned.identity.runId, claimed.resumeToken)
        ?.count,
      0,
    );
    const reportedWithAdapterOutput = await reportEnnoWork(database, {
      ...
      result: {
        outcome: "completed",
        summary: "Used only adapter continuation credentials",
        mutated: false,
        changedPaths: [],
      },
    });
    ...
    await assert.rejects(
      prepareEnnoVerification(database, {
    ...
    const refreshedRoute = decideAdapterContinuation(database, "opencode", {
      sessionId: "latest-opencode-session",
      cwd: root,
    });
    assert.ok(refreshedRoute.resumeToken);
    const replayedWithRotatedCredentials = await prepareEnnoVerification(
      database,
      {
        runId: planned.identity.runId,
...
test("hook prefers an exact session and otherwise refuses ambiguous active repository runs", async () => {
  const { root, database } = await fixture();
  try {
    const first = await plannedExecution(
      database,
      root,
      "pending-first",
      verifier(root, "first"),
    );
    const second = await plannedExecution(
      database,
      root,
      "pending-second",
      verifier(root, "second"),
      {
        clientIdentity: "kind_only",
      },
    );
    ...
    assert.deepEqual(
      ...
      new Map([
        [
          ...
          {
            clientSessionId: first.hostSessionId,
            status: "goki_executing",
            ledgerStatus: "active",
          },
        ],
        [
          second.identity.runId,
          {
...
test("an active WorkUnit lease prevents automatic rerouting to another local client", async () => {
  ...
  try {
    ...
    assert.throws(
      () =>
        decideAdapterContinuation(database, "claude", {
          session_id: "claude-local-session",
          cwd: root,
        }),
      /active Enno WorkUnit lease/iu,
    );
    const routedBinding = database
      .prepare(
        `
    ...
    assert.deepEqual(
      routedBinding === undefined ? undefined : { ...routedBinding },
      {
        clientKind: "codex",
        clientVersion: "codex-1.2.3",
        clientSessionId: planned.hostSessionId,
    ...
    await assert.rejects(
      reportEnnoWork(database, {
        ...
        result: {
          outcome: "completed",
          summary: "Expired owner must not report",
          mutated: false,
          changedPaths: [],
        },
      }),
    ...
    const completed = await reportEnnoWork(database, {
      ...
      result: {
        outcome: "completed",
        summary: "Recovered owner completed the WorkUnit",
        mutated: false,
        changedPaths: [],
      },
    });
...
test("Goki cannot start before Oduno derives the ideal and Zenki submits a plan", async () => {
  ...
  try {
    ...
    assert.equal(
      decideAdapterContinuation(database, "codex", {
        session_id: "codex-before-plan",
        cwd: root,
      }).directive?.role,
      "zenki",
    ...
    `,
        ...
        .get<{ count: number }>(prepared.run.runId)?.count,
      0,
    );
  } finally {
    database.close();
...
test("a missing plan environment catalog returns a recoverable choice without consuming the run", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "plan-environment-missing",
      cwd: root,
      task: "Repair the add function",
      profileHints: {
        taskType: "debug",
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-plan-environment-missing" },
      skillDiscoveryMode: "off",
    });
    const identity = {
      runId: prepared.run.runId,
    ...
    const { capabilities: omittedCapabilities, ...missingInput } =
      completeInput;
    assert.ok(omittedCapabilities.length > 0);
    await assert.rejects(
      submitEnnoPlan(database, {
        ...missingInput,
        idempotencyKey: "plan-environment-missing-first",
      }),
      (error: unknown) => {
        assert.ok(error instanceof KiokukoError);
        assert.equal(error.code, "CONFLICT");
        assert.deepEqual(error.details, {
          planStartRecoveryReason: "environment_information_missing",
        });
        return true;
      },
    );
    ...
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM agent_task_skill_discovery_attempts WHERE run_id = ? AND phase = 'zenki'",
        )
        .get<{ count: number }>(identity.runId)?.count,
      0,
    );
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM enno_advisory_rounds WHERE run_id = ?",
        )
        .get<{ count: number }>(identity.runId)?.count,
      0,
    );
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM ledger_events WHERE run_id = ? AND event_type = 'zenki.plan_created'",
        )
        .get<{ count: number }>(identity.runId)?.count,
      0,
    );
    assert.equal(
      decideAdapterContinuation(database, "codex", {
        session_id: "codex-plan-environment-missing",
        cwd: root,
      }).continue,
      false,
    );
...
test("a changed environment waits for a choice and explicit planning cancellation permits a clean restart", async () => {
  ...
  try {
    ...
    const prepared = await prepareAgentTask(database, {
      requestId: "plan-environment-changed",
      cwd: root,
      task: "Repair the add function",
      profileHints: {
        taskType: "debug",
    ...
    await assert.rejects(
      submitEnnoPlan(database, changedInput),
      (error: unknown) => {
        assert.ok(error instanceof KiokukoError);
        assert.equal(error.code, "CONFLICT");
        assert.deepEqual(error.details, {
          planStartRecoveryReason: "environment_changed",
        });
        return true;
      },
    );
    ...
    assert.throws(
      () =>
        answerEnno(database, {
          ...identity,
          expectedRevision: 1,
          idempotencyKey: "planning-approve-forbidden",
          action: "approve",
        }),
      /not in the required state/iu,
    );
    assert.throws(
      () =>
        answerEnno(database, {
          ...identity,
          expectedRevision: 1,
          idempotencyKey: "planning-revise-forbidden",
          action: "revise",
    ...
    const cancelled = answerEnno(database, {
      ...identity,
      expectedRevision: 1,
      idempotencyKey: "planning-explicit-cancel",
      action: "cancel",
    });
    ...
    const restarted = await prepareAgentTask(database, {
      requestId: "plan-environment-restarted",
      cwd: root,
      task: "Repair the add function",
      profileHints: {
        taskType: "debug",
    ...
    submitPreparedIdeal(
      database,
      restarted,
      "plan-environment-restarted-ideal",
    );
    const continuation = decideAdapterContinuation(database, "codex", {
...
test("a legacy plan ended by a lost environment catalog returns restart choices without opening another run", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "legacy-plan-environment-ended",
      cwd: root,
      task: "Repair the add function",
      profileHints: {
        taskType: "debug",
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-legacy-plan-ended" },
      skillDiscoveryMode: "off",
    });
    const identity = {
      runId: prepared.run.runId,
    ...
    submitPreparedIdeal(
      database,
      prepared,
      "legacy-plan-environment-ended-ideal",
    );
    ...
    const runCountBefore = database
      .prepare("SELECT COUNT(*) AS count FROM ledger_runs")
      .get<{ count: number }>()?.count;

    await assert.rejects(
      submitEnnoPlan(database, { ...input, expectedRevision: 2 }),
      (error: unknown) => {
        assert.ok(error instanceof KiokukoError);
        assert.equal(error.code, "CONFLICT");
        assert.deepEqual(error.details, {
          planStartRecoveryReason: "previous_attempt_ended",
        });
        return true;
      },
    );
...
test("persisted WorkUnits validate the complete dependency graph during read-back", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "stored-work-unit-dependencies",
      cwd: root,
      task: "Build a dependent module pair",
      profileHints: {
        taskType: "debug",
        target: "src/prepare.js",
        expected: "tests pass",
        constraints: null,
      },
      capabilities,
      client: { kind: "codex", sessionId: "codex-stored-dependencies" },
      skillDiscoveryMode: "off",
    });
    const identity = {
      runId: prepared.run.runId,
    ...
    const planned = await submitEnnoPlan(database, {
      ...
      workPlan: {
        ...
        units: [
          {
            id: "prepare",
            objective: "Prepare the shared module",
            scope: ["src/prepare.js"],
            dependencies: [],
            routes: ["code"],
            skillNames: [],
            expertRefs: [
              {
                id: "code.domain.v1",
                reason: "Keep the prerequisite module contract deterministic",
              },
            ],
            acceptanceCriteria: ["tests pass"],
            focusedVerifiers: [],
          ...
          {
            id: "finalize",
            objective: "Finalize the dependent module",
            scope: ["src/finalize.js"],
            ...
            expertRefs: [
              {
                id: "code.verification.v1",
                reason:
                  "Verify the dependent module after its prerequisite completes",
              },
            ],
            acceptanceCriteria: ["tests pass"],
            focusedVerifiers: [],
      ...
      skillRequirements: [],
      finalVerifiers: [
        verifier(prepared.project.repositoryRoot, "stored-dependencies-final"),
      ],
      maxAttempts: 3,
      provenance: {
        scope: "explicit_user",
...
test("Zenki discovery uses a new plan digest and only the remaining run budget after replanning", async () => {
  ...
  try {
    ...
    const prepared = await prepareAgentTask(database, {
      requestId: "zenki-discovery-budget",
      cwd: root,
      ...
      client: { kind: "codex", sessionId: "codex-zenki-discovery-budget" },
    ...
    const discover = async (
      _database: Parameters<typeof discoverSkills>[0],
      input: Parameters<typeof discoverSkills>[1],
    ) => {
      calls.push({
        maxQueries: input.maxQueries,
        maxSelectedSkills: input.maxSelectedSkills,
        task: input.task,
      });
      return {
        attempted: true,
    ...
    const planInput = (
            ...
            expertRefs: [
              {
                id: "code.verification.v1",
                reason:
                  "Verify each replanned component repair with focused evidence",
              },
            ],
            acceptanceCriteria: ["tests pass"],
            focusedVerifiers: [],
      ...
      skillRequirements: [],
      finalVerifiers: [
        verifier(prepared.project.repositoryRoot, "zenki-discovery-final"),
      ],
      maxAttempts: 5,
      provenance: {
        scope: "explicit_user",
    ...
    const firstPlan = await submitEnnoPlan(
      database,
      planInput(1, "zenki-discovery-plan-1", "Repair the first component plan"),
      {
    ...
    await reportEnnoWork(database, {
      ...
      result: {
        outcome: "completed",
        summary: "Initial component repair",
        mutated: false,
        changedPaths: [],
      },
    });
    ...
    await assert.rejects(
      submitEnnoPlan(
        database,
        {
          ...planInput(
            3,
            "zenki-discovery-exhausted-plan",
            "Repair with an exhausted attempt budget",
          ),
          maxAttempts: 2,
        },
        { discoverSkills: discover },
    ...
    const secondPlan = await submitEnnoPlan(
      database,
      planInput(
        3,
        "zenki-discovery-plan-2",
        "Repair the narrower component plan",
      ),
      {
        discoverSkills: discover,
      },
    ...
    assert.deepEqual(
      calls.map((call) => ({
        maxQueries: call.maxQueries,
        maxSelectedSkills: call.maxSelectedSkills,
      })),
      [
    ...
    `,
      ...
      .all<{
        ...
        reservedSelections: number;
        consumedSelections: number;
      }>(identity.runId)
      .map((row) => ({ ...row }));
    assert.equal(attempts.length, 3);
    const zenkiAttempts = attempts.filter(
      (attempt) => attempt.phase === "zenki",
    );
    assert.equal(zenkiAttempts.length, 2);
    assert.notEqual(
      zenkiAttempts[0]?.requestDigest,
      zenkiAttempts[1]?.requestDigest,
    );
    assert.deepEqual(
      zenkiAttempts.map(
        ({ phase: _phase, requestDigest: _digest, ...budget }) => budget,
      ),
      [
        {
          reservedQueries: 3,
...
test("Zenki recovers migrated malformed-provider attempts without relaxing digest or budget boundaries", async (t) => {
  const prepare = async (requestId: string) => {
    ...
    const plan = {
        ...
        units: [
          {
            ...
            expertRefs: [
              {
                id: "code.verification.v1",
                reason: "Verify the recovered plan with focused evidence",
              },
            ],
            acceptanceCriteria: ["tests pass"],
            focusedVerifiers: [],
      ...
      skillRequirements: [],
      finalVerifiers: [
        verifier(prepared.project.repositoryRoot, `${requestId}-final`),
      ],
      maxAttempts: 5,
      provenance: {
        scope: "explicit_user" as const,
        exclusions: "explicit_user" as const,
        acceptanceCriteria: "explicit_user" as const,
        workPlan: "explicit_user" as const,
        skillSet: "explicit_user" as const,
        finalVerifiers: "explicit_user" as const,
        maxAttempts: "explicit_user" as const,
      },
      capabilities: discoveryCapabilities,
    };
    ...
    const insertFailedAttempt = (requestDigest: string) =>
  ...
  await t.test(
    "same digest replays a safe failure summary and exact operation replay remains stable",
    async () => {
      ...
      try {
        ...
        assert.equal(planned.ennoOduno.status, "goki_executing");
        assert.deepEqual(
          readEnnoSnapshot(context.database, context.identity).contract.skillSet
            .zenkiDiscovery,
          {
            attempted: false,
            mode: "official",
        ...
        assert.deepEqual(
          await submitEnnoPlan(context.database, context.plan),
          planned,
        );
        await assert.rejects(
          submitEnnoPlan(context.database, {
            ...context.plan,
            maxAttempts: 4,
          }),
      ...
      `,
            ...
            .get<{ state: string }>(context.identity.runId, context.digest)
            ?.state,
          "failed",
        );
      } finally {
  ...
  await t.test(
    "changed digest proceeds only with the remaining run-wide budget",
    async () => {
...
test("Zenki cannot submit a code WorkUnit without a selected expert fragment", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-missing-expert" },
      skillDiscoveryMode: "off",
    });
    ...
    await assert.rejects(
      submitEnnoPlan(database, {
        ...
        skillRequirements: [],
        finalVerifiers: [
          verifier(prepared.project.repositoryRoot, "missing-expert-final"),
        ],
        maxAttempts: 3,
        provenance: {
          scope: "explicit_user",
    ...
    assert.equal(
      database
        .prepare("SELECT revision FROM enno_contracts WHERE run_id = ?")
        .get<{ revision: number }>(prepared.run.runId)?.revision,
      1,
    );
    assert.equal(
      database
...
test("Oduno ideal requires one contribution for every Akinator-discovered Skill before Zenki starts", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "ideal-skill-coverage",
      cwd: root,
      task: "Repair the add function with discovered guidance",
      profileHints: {
        taskType: "debug",
        target: "src/add.js",
        expected: "tests pass",
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-ideal-coverage" },
      skillDiscoveryMode: "off",
    });
    const discovered = {
    ...
    assert.ok(stored);
    const contract = JSON.parse(stored.contractJson) as {
      skillSet: { intakeDiscovery: unknown };
    };
    contract.skillSet.intakeDiscovery = discovered;
    database
      .prepare(
        "UPDATE enno_contracts SET contract_json = ?, intake_discovery_json = ? WHERE run_id = ?",
      )
      .run(
        canonicalJson(contract),
        canonicalJson(discovered),
        prepared.run.runId,
      );
    const identity = {
      runId: prepared.run.runId,
    ...
    assert.equal(
      decideAdapterContinuation(database, "codex", {
        session_id: "codex-ideal-coverage",
        cwd: root,
      }).directive?.objective.includes("external-debug-reference"),
      true,
    );
    assert.throws(
      () =>
        ...
        skillContributions: [
          {
            ...
            contribution:
              "Use its diagnostic perspective as untrusted reference-only guidance",
          },
        ],
        successSignals: ["tests pass"],
      },
...
test("fake agent completes the Enno-Zenki-Goki loop in ledger order with fresh verifier evidence", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "enno-happy-path",
      cwd: root,
      task: "Fix the incorrect add function and make tests pass",
      profileHints: {
        taskType: "debug",
        target: "src/add.js",
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-session-1" },
      skillDiscoveryMode: "off",
    });
    assert.equal(prepared.ennoOduno.status, "oduno_ideal");
    assert.equal(prepared.ennoOduno.directive?.role, "enno-oduno");
    assert.equal(
      prepared.ennoOduno.directive?.handoff?.sourceRole,
      "enno-oduno",
    );
    ...
    assert.match(
      idealized.ennoOduno.ideal?.objective ?? "",
      /optimal verified outcome/iu,
    );
    const planningContext =
      idealized.ennoOduno.directive?.advisoryRound?.context;
    ...
    const plan = await submitEnnoPlan(database, {
            ...
            expertRefs: [
              {
                id: "code.verification.v1",
                reason:
                  "Prove the add regression through the focused test pipeline",
              },
            ],
      ...
      skillRequirements: [
        {
          name: "kiokuko-single-purpose-functions",
          purposes: ["implementation", "testing"],
          required: true,
        },
      ],
      finalVerifiers: [verifier(repositoryRoot, "final-test")],
      maxAttempts: 3,
      provenance: {
        scope: "explicit_user",
        exclusions: "explicit_user",
        acceptanceCriteria: "repository_evidence",
        workPlan: "repository_evidence",
        skillSet: "repository_evidence",
        finalVerifiers: "repository_evidence",
        maxAttempts: "explicit_user",
      },
      capabilities,
    });
    ...
    const hook = decideAdapterContinuation(database, "codex", {
      session_id: "codex-session-1",
      cwd: root,
    });
    assert.equal(hook.continue, true);
    assert.equal(hook.directive?.role, "goki");
    assert.deepEqual(hook.directive?.requiredSkills, [
      "kiokuko-soul",
      "kiokuko-single-purpose-functions",
    ]);
    ...
    const worked = await reportEnnoWork(database, {
      ...identity,
      ...executionCredentials(hook),
      expectedRevision: 2,
      idempotencyKey: "work-1",
      workUnitId: "repair-add",
      result: {
        outcome: "completed",
        summary: "Fixed add and added coverage",
        mutated: true,
    ...
    const verified = await prepareEnnoVerification(database, {
      ...identity,
      expectedRevision: 2,
      idempotencyKey: "verify-prepare-1",
    });
    ...
    const finalReviewContext =
      verified.ennoOduno.directive?.advisoryRound?.context;
    ...
    assert.equal(
      finalAdvice.advisoryRound.inputDigest,
      advisoryDigest("final_review", 2, 1, finalReviewContext),
    );

    let finishSpawnCalls = 0;
    const finished = await finishEnno(
      database,
      {
        ...identity,
        expectedRevision: 2,
    ...
    assert.match(
      finished.ennoOduno.directive?.objective ?? "",
      /obsolete, useless, or redundant tests and functions/iu,
    );
    assert.equal(finished.verifierResults?.[0]?.status, "passed");
    assert.deepEqual(
      await finishEnno(database, {
        ...identity,
        expectedRevision: 2,
        idempotencyKey: "finish-1",
        advisoryRoundDigest: finalAdvice.advisoryRound.inputDigest,
    ...
    await assert.rejects(
      finishEnno(database, {
        ...
        review: {
          decision: "replan",
          summary: "Changed Review input must not replay",
        },
      }),
      /idempotency key was reused with different input/iu,
    );

    const meditationContinuation = decideAdapterContinuation(
      database,
      "claude",
      {
        session_id: "claude-after-finish",
        cwd: root,
      },
    );
    ...
    assert.match(
      meditationContinuation.directive?.objective ?? "",
      /obsolete, useless, or redundant tests and functions/iu,
    );
    ...
    const deletionCandidates = [
      {
        ...
        name: "legacyAdd",
        reason:
          "The verified implementation supersedes this unused compatibility helper",
        evidence: ["No approved WorkUnit or verifier depends on legacyAdd"],
      },
    ];
    const meditated = submitMeditation(
      database,
      identity,
      2,
      "meditation-1",
    ...
    assert.deepEqual(
      meditated.ennoOduno.meditation?.deletionCandidates,
      deletionCandidates,
    );
    assert.deepEqual(
      submitMeditation(
    ...
    `,
      ...
      .all<{ eventType: string }>(identity.runId)
    ...
    `,
      ...
      .all<{ revision: number; mutationRevision: number; status: string }>(
        identity.runId,
      );
    assert.deepEqual(
    ...
    assert.deepEqual(
      evidence.map((item) => item.mutationRevision),
      [1, 1],
    );
  } finally {
    database.close();
...
test("a needs_confirmation plan presents a complete user-facing confirmation projection", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-confirmation" },
      skillDiscoveryMode: "off",
    });
    const repositoryRoot = prepared.project.repositoryRoot;
    const identity = {
    ...
    const plan = await submitEnnoPlan(
      database,
      confirmationProjectionPlanInput(
        identity,
        repositoryRoot,
        "projection-plan-1",
        "Repair add behind the confirmation",
        {
          scope: "explicit_user",
          exclusions: "explicit_user",
          acceptanceCriteria: "explicit_user",
    ...
    assert.match(
      plan.ennoOduno.directive?.objective ?? "",
      /Return every item in userFacingConfirmation to the user in the user's language/iu,
    );
    assert.deepEqual(plan.ennoOduno.directive?.reportSchema.required, [
      "runId",
      "expectedRevision",
      "idempotencyKey",
      "action",
    ...
    assert.deepEqual(projection.summary, {
      basis: "proposal",
      text: "Repair add behind the confirmation",
    });
    ...
    assert.deepEqual(projection.attemptLimit, {
      basis: "proposal",
      maxAttempts: 5,
    });
    assert.equal(
      projection.skills.every((skill) => skill.referenceOnly === false),
      true,
    );
    ...
    for (const forbidden of [
      ...
      assert.equal(
        ...
        false,
        `projection leaked internal token: ${forbidden}`,
      );
    }

    const replay = await submitEnnoPlan(
      database,
      confirmationProjectionPlanInput(
        identity,
        repositoryRoot,
    ...
    const stale = answerEnno(database, {
      ...identity,
      expectedRevision: 2,
      idempotencyKey: "projection-stale-approve",
      action: "approve",
    });
    assert.throws(
      () =>
        answerEnno(database, {
          ...identity,
          expectedRevision: 99,
          idempotencyKey: "projection-older-approve",
          action: "approve",
        }),
      /revision changed/iu,
    );
    ...
    assert.equal(
      "userFacingConfirmation" in (stale.ennoOduno.directive ?? {}),
      false,
    );
  } finally {
    database.close();
...
test("revise returns to Zenki for a fresh projection and cancel terminates without one", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "confirmation-revise-cancel",
      cwd: root,
      task: "Repair the add function",
      profileHints: {
        taskType: "debug",
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-revision" },
      skillDiscoveryMode: "off",
    });
    const repositoryRoot = prepared.project.repositoryRoot;
    const identity = {
    ...
    const revised = answerEnno(database, {
      ...identity,
      expectedRevision: 2,
      idempotencyKey: "revision-revise",
      action: "revise",
    ...
    assert.equal(
      "userFacingConfirmation" in (revised.ennoOduno.directive ?? {}),
      false,
    );
    const replanned = await submitEnnoPlan(database, {
      ...confirmationProjectionPlanInput(
        ...
        repositoryRoot,
        "revision-plan-2",
        "Repair add with a narrower scope",
        provenance,
      ),
      expectedRevision: 3,
    });
    ...
    assert.equal(
      replanned.ennoOduno.directive?.userFacingConfirmation?.summary.text,
      "Repair add with a narrower scope",
    );
    const cancelled = answerEnno(database, {
      ...identity,
      expectedRevision: 4,
      idempotencyKey: "revision-cancel",
      action: "cancel",
    });
    assert.equal(cancelled.ennoOduno.status, "cancelled");
...
test("an all-explicit plan skips confirmation and carries no projection", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "explicit-skips-confirmation",
      cwd: root,
      task: "Repair the add function",
      profileHints: {
        taskType: "debug",
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-explicit" },
      skillDiscoveryMode: "off",
    });
    const repositoryRoot = prepared.project.repositoryRoot;
    const identity = {
    ...
    const plan = await submitEnnoPlan(
      ...
      confirmationProjectionPlanInput(
        ...
        repositoryRoot,
        "explicit-plan-1",
        "Repair add without confirmation",
        {
          scope: "explicit_user",
          exclusions: "explicit_user",
          acceptanceCriteria: "explicit_user",
    ...
    assert.equal(
      "userFacingConfirmation" in (plan.ennoOduno.directive ?? {}),
      false,
    );
  } finally {
    database.close();
...
test("a pre-migration active run without an Oduno ideal keeps its legacy completion path", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "legacy-active-run",
      cwd: root,
      task: "Repair a legacy active run",
      profileHints: {
        taskType: "debug",
        target: "src/add.js",
        expected: "tests pass",
      ...
      capabilities,
      client: { kind: "codex", sessionId: "codex-legacy-active" },
      skillDiscoveryMode: "off",
    });
    database
      .prepare("UPDATE enno_contracts SET phase = NULL WHERE run_id = ?")
      .run(prepared.run.runId);
    const identity = {
      runId: prepared.run.runId,
      workspace: prepared.project.workspace,
      orchestrationId: prepared.intake.sessionId,
    ...
    const planned = await submitEnnoPlan(database, {
      ...
      acceptanceCriteria: [{ id: "tests", description: "tests pass" }],
      workPlan: {
        objective: "Repair the legacy run",
        units: [
          {
            id: "repair",
            objective: "Repair add",
            ...
            expertRefs: [
              {
                id: "code.verification.v1",
                reason: "Verify the legacy repair through its matching test",
              },
            ],
            acceptanceCriteria: ["tests pass"],
            focusedVerifiers: [],
      ...
      skillRequirements: [],
      finalVerifiers: [
        verifier(prepared.project.repositoryRoot, "legacy-final"),
      ],
      maxAttempts: 3,
      provenance: {
        scope: "explicit_user",
    ...
    await reportEnnoWork(database, {
      ...
      expectedRevision: 2,
      idempotencyKey: "legacy-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Repaired legacy run",
        mutated: true,
        changedPaths: ["src/add.js"],
      },
    });
    database
      .prepare(
        "UPDATE enno_work_units SET result_json = ? WHERE run_id = ? AND work_unit_id = ?",
      )
      .run(
        canonicalJson({
          outcome: "completed",
          ...
          mutated: true,
          changedPaths: [
            path.join(prepared.project.repositoryRoot, "src", "add.js"),
          ],
        }),
        identity.runId,
        "repair",
    ...
    assert.deepEqual(
      readEnnoSnapshot(database, identity).workUnits[0]?.result?.changedPaths,
      ["src/add.js"],
    );
...
test("mixed UI, code, test, docs, and operations WorkUnits route experts and Skills locally", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "enno-code-ui-skills",
      cwd: root,
      task: "Build an accessible settings panel",
      profileHints: {
        taskType: "build",
        target: "src/Settings.tsx",
        expected: "UI tests pass",
        constraints: null,
      },
      capabilities,
      client: { kind: "codex", sessionId: "codex-code-ui" },
      skillDiscoveryMode: "off",
    });
    const identity = {
      runId: prepared.run.runId,
    ...
    const plan = await submitEnnoPlan(database, {
      ...
      workPlan: {
        objective: "Build the accessible settings panel",
        units: [
          {
            id: "settings-ui",
            objective: "Implement the settings panel",
            scope: ["src/Settings.tsx"],
            dependencies: [],
            routes: ["ui"],
            skillNames: ["kiokuko-ui-design-soul", "kiokuko-soul"],
            acceptanceCriteria: ["UI tests pass"],
            focusedVerifiers: [],
            expertRefs: [
              {
                id: "code.domain.v1",
                reason: "Keep settings state transitions deterministic",
              },
              {
                id: "ui.accessibility.v1",
                reason:
                  "The panel must support accessible labels, focus, and keyboard use",
              },
            ],
          },
          {
            id: "catalog",
            objective: "Convert the settings catalog",
            scope: ["src/catalog.ts"],
            dependencies: ["settings-ui"],
            routes: ["code"],
            skillNames: [],
            acceptanceCriteria: ["Catalog conversion is deterministic"],
            focusedVerifiers: [],
            expertRefs: [
              { id: "code.domain.v1", reason: "Preserve catalog invariants" },
            ],
          },
          {
            id: "tests",
            objective: "Add regression tests",
            scope: ["tests/settings.test.ts"],
          ...
          {
            id: "docs",
            objective: "Update usage documentation",
            scope: ["README.md"],
            dependencies: ["tests"],
            routes: ["docs"],
            skillNames: [],
            expertRefs: [],
            acceptanceCriteria: ["Documentation matches behavior"],
            focusedVerifiers: [],
          },
          {
            id: "residual",
            objective: "Remove residual references",
            scope: [".gitignore"],
            dependencies: ["docs"],
            routes: ["operations"],
            skillNames: [],
            expertRefs: [],
            acceptanceCriteria: ["No stale references remain"],
            focusedVerifiers: [],
          },
        ],
      },
      skillRequirements: [
        {
          name: "kiokuko-ui-design-soul",
          purposes: ["ui", "implementation", "testing"],
          required: true,
        },
      ],
      finalVerifiers: [
        verifier(prepared.project.repositoryRoot, "code-ui-final"),
      ],
      maxAttempts: 8,
      provenance: {
        scope: "explicit_user",
    ...
    assert.deepEqual(
      plan.ennoOduno.directive?.workUnit?.expertRefs.map(
        (reference) => reference.id,
      ),
      ["code.domain.v1", "ui.accessibility.v1"],
    );

    const catalog = await reportEnnoWork(database, {
      ...identity,
      ...executionCredentials(plan),
      expectedRevision: 2,
      idempotencyKey: "code-ui-work",
      workUnitId: "settings-ui",
      result: {
        outcome: "completed",
        summary: "Implemented the accessible settings panel",
        mutated: true,
        changedPaths: ["src/Settings.tsx"],
      },
    });
    ...
    const tests = await reportEnnoWork(database, {
      ...identity,
      ...executionCredentials(catalog),
      expectedRevision: 2,
      idempotencyKey: "code-catalog-work",
      workUnitId: "catalog",
      result: {
        outcome: "completed",
        summary: "Converted the settings catalog",
        mutated: true,
        changedPaths: ["src/catalog.ts"],
      },
    });
    assert.equal(tests.ennoOduno.directive?.workUnit?.id, "tests");
    assert.deepEqual(tests.ennoOduno.directive?.requiredSkills, [
      "kiokuko-soul",
    ]);
    const docs = await reportEnnoWork(database, {
      ...identity,
      ...executionCredentials(tests),
      expectedRevision: 2,
      idempotencyKey: "code-tests-work",
      workUnitId: "tests",
      result: {
        outcome: "completed",
        summary: "Added regression tests",
        mutated: true,
        changedPaths: ["tests/settings.test.ts"],
      },
    });
    assert.equal(docs.ennoOduno.directive?.workUnit?.id, "docs");
    assert.deepEqual(docs.ennoOduno.directive?.requiredSkills, [
      "kiokuko-soul",
    ]);
    const residual = await reportEnnoWork(database, {
      ...identity,
      ...executionCredentials(docs),
      expectedRevision: 2,
      idempotencyKey: "code-docs-work",
      workUnitId: "docs",
      result: {
        outcome: "completed",
        summary: "Updated usage documentation",
        mutated: true,
        changedPaths: ["README.md"],
      },
    });
    assert.equal(residual.ennoOduno.directive?.workUnit?.id, "residual");
    assert.deepEqual(residual.ennoOduno.directive?.requiredSkills, [
      "kiokuko-soul",
    ]);
    const reviewed = await reportEnnoWork(database, {
      ...identity,
      ...executionCredentials(residual),
      expectedRevision: 2,
      idempotencyKey: "code-residual-work",
      workUnitId: "residual",
      result: {
        outcome: "completed",
        summary: "Removed residual references",
        mutated: true,
...
test("an Enno plan blocks when the exact local kiokuko-soul capability is absent", async () => {
  ...
  try {
    const capabilitiesWithoutSoul = [
      {
        ...
        name: "kiokuko_soul",
        description:
          "A non-canonical alias must not satisfy the master contract.",
      },
      ...capabilities.filter(
        (capability) => capability.name !== "kiokuko-soul",
      ),
    ];
    const prepared = await prepareAgentTask(database, {
      requestId: "enno-missing-soul",
      cwd: root,
      task: "Repair the add function",
      profileHints: {
        taskType: "debug",
      ...
      client: { kind: "codex", sessionId: "codex-missing-soul" },
      skillDiscoveryMode: "off",
    });
    ...
    const blocked = await submitEnnoPlan(database, {
      ...
      orchestrationId: prepared.intake.sessionId,
      expectedRevision: 1,
      idempotencyKey: "missing-soul-plan",
      scope: ["src/add.js"],
      exclusions: [],
      acceptanceCriteria: [{ id: "tests", description: "tests pass" }],
      workPlan: {
            ...
            expertRefs: [
              {
                id: "code.verification.v1",
                reason:
                  "Verify the repair while testing the missing required Skill gate",
              },
            ],
            acceptanceCriteria: ["tests pass"],
            focusedVerifiers: [],
      ...
      skillRequirements: [],
      finalVerifiers: [
        verifier(prepared.project.repositoryRoot, "missing-soul-final"),
      ],
      maxAttempts: 3,
      provenance: {
        scope: "explicit_user",
    ...
    assert.match(
      database
        .prepare("SELECT blocker FROM enno_contracts WHERE run_id = ?")
        .get<{ blocker: string }>(prepared.run.runId)?.blocker ?? "",
      /Required Skills unavailable: kiokuko-soul/u,
    );
    assert.equal(
      database
        .prepare("SELECT status FROM ledger_runs WHERE run_id = ?")
        .get<{ status: string }>(prepared.run.runId)?.status,
      "failed",
    );
  } finally {
    database.close();
...
test("run identity rejects cross-run progress while trusted repository routing crosses sessions and clients", async () => {
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      requestId: "enno-session-binding",
      cwd: root,
      ...
      capabilities,
      client: { kind: "claude", sessionId: "claude-owner" },
      skillDiscoveryMode: "off",
    });
    const identity = {
      runId: prepared.run.runId,
      workspace: prepared.project.workspace,
      orchestrationId: "not-owner",
    };
    await assert.rejects(
      submitEnnoPlan(database, {
        ...identity,
              ...
              expertRefs: [
                {
                  id: "code.domain.v1",
                  reason:
                    "Implement the module as deterministic domain behavior",
                },
              ],
              acceptanceCriteria: ["tests pass"],
              focusedVerifiers: [],
        ...
        skillRequirements: [],
        finalVerifiers: [verifier(root, "test")],
        maxAttempts: 2,
        provenance: {
          scope: "explicit_user",
          exclusions: "explicit_user",
    ...
    const sameClient = decideAdapterContinuation(database, "claude", {
      session_id: "another-session",
      cwd: root,
    });
    assert.equal(sameClient.continue, true);
    assert.equal(sameClient.runId, prepared.run.runId);
    const crossClient = decideAdapterContinuation(database, "codex", {
...
test("failed Enno review returns to Zenki for a revision-bound replan before Goki can resume", async () => {
  ...
  try {
    ...
    const conditional = {
      ...
      args: [
        "--eval",
        `import('node:fs').then(({existsSync}) => process.exit(existsSync(${JSON.stringify(marker)}) ? 0 : 1))`,
      ],
      cwd: root,
      timeoutMs: 5000,
    };
    ...
    await reportEnnoWork(database, {
      ...
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "fresh-work-1",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Initial repair",
        mutated: true,
        changedPaths: ["src/add.js"],
      },
    });
    ...
    assert.ok(
      failed.ennoOduno.directive?.requiredSkills.includes(
        "kiokuko-single-purpose-functions",
      ),
    );
    assert.match(
      failed.ennoOduno.directive?.objective ?? "",
      /review rejected contract revision 2/iu,
    );
    assert.match(
      failed.ennoOduno.directive?.objective ?? "",
      /focused runnable test target/iu,
    ...
    const planningContinuation = decideAdapterContinuation(database, "codex", {
      session_id: "codex-fresh-evidence",
      cwd: root,
    });
    assert.equal(planningContinuation.continue, true);
    ...
    await assert.rejects(
      reportEnnoWork(database, {
        ...identity,
        expectedRevision: 3,
        idempotencyKey: "illegal-old-plan-resume",
        workUnitId: "repair",
        result: {
          outcome: "completed",
          summary: "Old plan cannot resume",
          mutated: false,
          changedPaths: [],
        },
      }),
    ...
    const replanned = await submitEnnoPlan(database, {
      ...identity,
      expectedRevision: 3,
      idempotencyKey: "fresh-replan",
      scope: ["src/add.js"],
      exclusions: [],
      acceptanceCriteria: [{ id: "tests", description: "tests pass" }],
      workPlan: {
        objective: "Replan repair after Enno review",
        units: [
          {
            id: "repair",
            objective: "Repair the final verification failure",
            scope: ["src/add.js"],
            dependencies: [],
            routes: ["code"],
            skillNames: [],
            expertRefs: [
              {
                id: "code.verification.v1",
                reason: "Address the failed final verifier with fresh evidence",
              },
            ],
            acceptanceCriteria: ["tests pass"],
            focusedVerifiers: [],
      ...
      skillRequirements: [],
      finalVerifiers: [{ ...conditional, cwd: "." }],
      maxAttempts: 5,
      provenance: {
        scope: "explicit_user",
        exclusions: "explicit_user",
    ...
    await reportEnnoWork(database, {
      ...identity,
      ...executionCredentials(replanned),
      expectedRevision: 4,
      idempotencyKey: "fresh-work-2",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Repaired final failure",
        mutated: true,
        changedPaths: ["src/add.js"],
      },
    });
    ...
    `,
      ...
      .all<{ contractRevision: number; workUnitId: string; status: string }>(
        identity.runId,
      )
      .map((row) => ({ ...row }));
...
test("Enno can reject passing verifier evidence and require Zenki to replan", async () => {
  ...
  try {
    ...
    await reportEnnoWork(database, {
      ...
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "review-reject-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Implemented the approved unit",
        mutated: true,
        changedPaths: ["src/add.js"],
      },
    });
    await prepareEnnoVerification(database, {
      ...identity,
      expectedRevision: 2,
      idempotencyKey: "review-reject-prepare",
    });
    const rejected = await finishEnno(database, {
      ...identity,
      expectedRevision: 2,
      idempotencyKey: "review-reject-finish",
      review: {
        decision: "replan",
        summary:
          "The public API acceptance criterion is not covered by the plan",
      },
    });
    ...
    assert.match(
      rejected.ennoOduno.directive?.objective ?? "",
      /public API acceptance criterion/iu,
    );
    assert.equal(
      database
        .prepare(
    ...
    `,
        ...
        .all<{ eventType: string }>(identity.runId)
        .map((row) => row.eventType),
      ["enno.verification_passed", "enno.replan_requested"],
    );
  } finally {
    database.close();
  }
...
test("replanning invalidates old final evidence and advisory digests", async () => {
  ...
  try {
    const planned = await plannedExecution(
      ...
      root,
      "stale-final-review",
      verifier(root, "stale-final"),
    );
    await reportEnnoWork(database, {
      ...planned.identity,
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "stale-initial-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Initial work is complete",
        mutated: false,
        changedPaths: [],
      },
    });
    await assert.rejects(
      finishEnno(database, {
        ...planned.identity,
        expectedRevision: 2,
        idempotencyKey: "unprepared-finish",
        review: {
          decision: "accept",
          summary: "Attempt to accept without prepared evidence",
        },
      }),
      /final verification evidence is not prepared/iu,
    );
    assert.equal(
      readEnnoSnapshot(database, planned.identity).status,
      "enno_verifying",
    );
    const prepared = await prepareEnnoVerification(database, {
      ...planned.identity,
      expectedRevision: 2,
      idempotencyKey: "stale-initial-prepare",
    });
    ...
    const replanned = await finishEnno(database, {
      ...
      review: {
        decision: "replan",
        summary: "Require a fresh contract review",
      },
    });
    ...
    const nextPlan = await submitEnnoPlan(database, {
      ...
      acceptanceCriteria: [{ id: "tests", description: "tests pass" }],
      workPlan: {
        objective: "Replan after stale review",
        units: [
          {
            id: "repair",
            objective: "Repair add with fresh review",
            scope: ["src/add.js"],
            dependencies: [],
            routes: ["code"],
            skillNames: [],
            expertRefs: [
              {
                id: "code.verification.v1",
                reason: "Verify the revised contract with fresh evidence",
              },
            ],
            acceptanceCriteria: ["tests pass"],
            focusedVerifiers: [],
    ...
    await reportEnnoWork(database, {
      ...planned.identity,
      ...executionCredentials(nextPlan),
      expectedRevision: 4,
      idempotencyKey: "stale-replanned-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Replanned work is complete",
        mutated: false,
        changedPaths: [],
      },
    });
    await assert.rejects(
      finishEnno(database, {
        ...planned.identity,
        expectedRevision: 4,
        idempotencyKey: "stale-evidence-finish",
        review: {
          decision: "accept",
          summary: "Attempt to reuse old evidence",
        },
      }),
      /final verification evidence is not prepared/iu,
    );
    ...
    const fresh = await prepareEnnoVerification(database, {
      ...planned.identity,
      expectedRevision: 4,
      idempotencyKey: "stale-replanned-prepare",
    });
    ...
    await assert.rejects(
      finishEnno(database, {
        ...
        review: {
          decision: "accept",
          summary: "Attempt to reuse old advisory digest",
        },
      }),
      /Enno input is invalid/iu,
    );
    ...
    const finished = await finishEnno(database, {
      ...planned.identity,
      expectedRevision: 4,
      idempotencyKey: "stale-fresh-finish",
...
test("spawn failure blocks the run while continuation exhaustion only stops the current session", async () => {
  ...
  try {
    const unsafe = {
      ...verifier(first.root, "missing"),
      executable: "kiokuko-executable-that-does-not-exist",
    };
    const planned = await plannedExecution(
      first.database,
      first.root,
  ...
  try {
    const planned = await plannedExecution(
      ...
      second.root,
      "continuation-limit",
      verifier(second.root, "pass"),
      { maxAttempts: 1 },
    );
    assert.equal(
      decideAdapterContinuation(second.database, "codex", {
        session_id: planned.hostSessionId,
        cwd: second.root,
      }).continue,
      true,
    );
    const exhausted = decideAdapterContinuation(second.database, "codex", {
      session_id: planned.hostSessionId,
      cwd: second.root,
    });
    ...
    assert.equal(
      second.database
        .prepare("SELECT status FROM enno_contracts WHERE run_id = ?")
        .get<{ status: string }>(planned.identity.runId)?.status,
      "goki_executing",
    );
    assert.equal(
      second.database
    ...
    assert.throws(
      () =>
        decideAdapterContinuation(second.database, "codex", {
          session_id: "replacement-codex-session",
          cwd: second.root,
        }),
      /active Enno WorkUnit lease/iu,
    );
...
test("timeout final evidence cannot be accepted", async () => {
  ...
  try {
    ...
    await reportEnnoWork(database, {
      ...
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "timeout-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Ready for timeout verification",
        mutated: false,
        changedPaths: [],
      },
    });
    ...
    const rejected = await finishEnno(database, {
      ...planned.identity,
      expectedRevision: 2,
      idempotencyKey: "timeout-finish",
      review: {
        decision: "accept",
        summary: "Timeout evidence must not be accepted",
      },
    });
    assert.equal(rejected.ennoOduno.status, "zenki_planning");
    assert.equal(rejected.ennoOduno.contractRevision, 3);
...
test("Claude returns control before its native eighth consecutive Stop block override", async () => {
  ...
  try {
    ...
    `,
        ...
        .get<{ totalCount: number }>(planned.identity.runId)?.totalCount,
      7,
    );
    assert.equal(
      database
    ...
    assert.throws(
      () =>
        decideAdapterContinuation(database, "opencode", {
          session_id: "opencode-after-claude-limit",
          cwd: root,
        }),
      /active Enno WorkUnit lease/iu,
    );
...
test("focused verifier process can write the same database because no transaction is held while it runs", async () => {
  ...
  try {
    ...
    const probe = {
      ...
      args: [
        "--eval",
        `import('node:sqlite').then(({DatabaseSync}) => { const db = new DatabaseSync(${JSON.stringify(databasePath)}); db.exec("INSERT INTO enno_lock_probe VALUES ('ok')"); db.close(); })`,
      ],
      cwd: root,
      timeoutMs: 5000,
    };
    ...
    const reported = await reportEnnoWork(database, {
      ...
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "lock-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Run lock probe",
        mutated: false,
        changedPaths: [],
      },
    });
    ...
    assert.equal(
      database
        .prepare("SELECT COUNT(*) AS count FROM enno_lock_probe")
        .get<{ count: number }>()?.count,
      1,
    );
  } finally {
...
test("concurrent work reports allow only one revision-bound state transition", async () => {
  ...
  try {
    ...
    const report = (idempotencyKey: string) =>
      reportEnnoWork(database, {
        ...planned.identity,
        ...executionCredentials(planned),
        expectedRevision: 2,
    ...
    assert.match(
      ...
      /another Enno operation is already in progress|state changed concurrently|expected goki_executing|not active/iu,
    );
    ...
    assert.throws(
      () =>
        withImmediateTransaction(database, () =>
          updateContractInTransaction(database, staleSnapshot, {
...
test("expired operation and verifier owners are abandoned before an atomic reclaim", async () => {
  ...
  try {
    const planned = await plannedExecution(
      ...
      root,
      "crash-recovery",
      verifier(root, "crash-recovery-final"),
    );
    const operation = {
    ...
    const firstOwner = withImmediateTransaction(database, () =>
      startOperationInTransaction(database, planned.identity.runId, operation),
    );
    database
      .prepare(
        `
    ...
    assert.equal(
      readOperationReceipt(database, planned.identity.runId, operation),
      undefined,
    );
    assert.equal(
      database
    ...
    assert.throws(
      () =>
        readOperationReceipt(database, planned.identity.runId, {
          ...operation,
          requestDigest: "b".repeat(64),
        }),
      /different input/iu,
    );
    const secondOwner = withImmediateTransaction(database, () =>
      startOperationInTransaction(database, planned.identity.runId, operation),
    );
    ...
    assert.equal(
      database
        .prepare(
          "SELECT status FROM enno_verifier_runs WHERE verifier_run_id = ?",
        )
        .get<{ status: string }>(abandonedVerifierId)?.status,
      "abandoned",
    );
    assert.equal(
      database
        ...
        .get<{ status: string }>(reclaimedVerifierId)?.status,
      "started",
    );
  } finally {
    database.close();
...
test("work-report secrets are sanitized before result, event, and receipt persistence", async () => {
  ...
  try {
    const planned = await plannedExecution(
      database,
      root,
      "work-secret-sanitization",
      verifier(root, "work-secret-final"),
    );
    ...
    const persisted = {
      ...
      `,
        ...
        .get<{ value: string }>(planned.identity.runId)?.value,
      blocker: database
        .prepare("SELECT blocker AS value FROM enno_contracts WHERE run_id = ?")
...
test("final verifier evidence is reused for the same revision and mutation without a second subprocess", async () => {
  ...
  try {
    const planned = await plannedExecution(
      ...
      verifier(root, "final-replay"),
    );
    await reportEnnoWork(database, {
      ...planned.identity,
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "final-replay-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Prepared final verification",
        mutated: false,
        changedPaths: [],
      },
    });
    ...
    const verifierRunIds = startVerifierRunsInTransaction(database, {
      ...
      repositoryEvidence: {
        ...
        verifierSpecDigest: canonicalContentHash(
          current.contract.finalVerifiers,
        ),
      },
    });
    finishVerifierRunsInTransaction(
    ...
    const finished = await finishEnno(
      database,
      {
        ...planned.identity,
        expectedRevision: 2,
        idempotencyKey: "final-replay-finish",
        review: {
          decision: "accept",
          summary: "Reuse the fresh final evidence",
        },
      },
      {
        spawn: (() => {
...
test("final verification preparation reuses fresh evidence across keys and finish never spawns", async () => {
  ...
  try {
    const planned = await plannedExecution(
      database,
      root,
      "final-evidence-cross-key",
      verifier(root, "final-cross-key"),
    );
    await reportEnnoWork(database, {
      ...planned.identity,
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "cross-key-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Prepared final verification",
    ...
    const countingSpawn = ((...args: unknown[]) => {
      spawnCalls += 1;
      return (
        nodeSpawn as unknown as (
          ...items: unknown[]
        ) => ReturnType<typeof nodeSpawn>
      )(...args);
    }) as unknown as typeof nodeSpawn;
    const first = await prepareEnnoVerification(
      database,
      {
        ...planned.identity,
        expectedRevision: 2,
        idempotencyKey: "cross-key-prepare-1",
      },
      { spawn: countingSpawn },
    );
    assert.equal(first.verifierResults?.[0]?.status, "passed");
    assert.equal(spawnCalls, 1);
    const second = await prepareEnnoVerification(
      database,
      {
        ...planned.identity,
        expectedRevision: 2,
        idempotencyKey: "cross-key-prepare-2",
      },
      { spawn: countingSpawn },
    );
    ...
    const finished = await finishEnno(
      database,
      {
        ...planned.identity,
        expectedRevision: 2,
        idempotencyKey: "cross-key-finish",
...
test("final evidence is invalidated by repository changes after preparation", async () => {
  ...
  try {
    ...
    const planned = await plannedExecution(
      database,
      root,
      "repository-evidence-stale",
      verifier(root, "repository-evidence-final"),
    );
    await reportEnnoWork(database, {
      ...planned.identity,
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "repository-evidence-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Prepared work for repository evidence",
        mutated: false,
        changedPaths: [],
      },
    });
    const prepared = await prepareEnnoVerification(database, {
      ...planned.identity,
      expectedRevision: 2,
      idempotencyKey: "repository-evidence-prepare",
    });
    assert.equal(prepared.verifierResults?.[0]?.status, "passed");
    assert.equal(
      prepared.verifierResults?.[0]?.changedDuringVerification,
      false,
    );
    ...
    await assert.rejects(
      finishEnno(database, {
        ...planned.identity,
        expectedRevision: 2,
        idempotencyKey: "repository-evidence-finish",
        review: {
          decision: "accept",
          summary: "Attempt to accept stale evidence",
        },
      }),
      /final verification evidence is not prepared/iu,
    );
    ...
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM enno_operation_receipts WHERE run_id = ? AND operation = 'finish'",
        )
        .get<{ count: number }>(planned.identity.runId)?.count,
      0,
    );
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM ledger_events WHERE run_id = ? AND event_type = 'enno.review_started'",
        )
        .get<{ count: number }>(planned.identity.runId)?.count,
      0,
    );
...
test("a verifier that mutates repository state cannot produce acceptable final evidence", async () => {
  ...
  try {
    const mutatingVerifier = {
      ...
      args: [
        "--eval",
        "require('node:fs').writeFileSync('verifier-mutated.txt', 'changed')",
      ],
      cwd: ".",
      timeoutMs: 5_000,
    };
    ...
    await reportEnnoWork(database, {
      ...
      ...executionCredentials(planned),
      expectedRevision: 2,
      idempotencyKey: "repository-mutation-work",
      workUnitId: "repair",
      result: {
        outcome: "completed",
        summary: "Prepared work for a mutating verifier",
        mutated: false,
        changedPaths: [],
      },
    });
    const prepared = await prepareEnnoVerification(database, {
      ...planned.identity,
      expectedRevision: 2,
      idempotencyKey: "repository-mutation-prepare",
    });
    assert.equal(prepared.verifierResults?.[0]?.status, "passed");
    assert.equal(
      prepared.verifierResults?.[0]?.changedDuringVerification,
    ...
    assert.equal(
      readEnnoSnapshot(database, planned.identity).finalEvidenceReady,
      false,
    );
    await assert.rejects(
      finishEnno(database, {
        ...planned.identity,
        expectedRevision: 2,
        idempotencyKey: "repository-mutation-finish",
        review: {
          decision: "accept",
          summary: "Mutating verifier evidence must be rejected",
        },
      }),
      /final verification evidence is not prepared/iu,
    );
