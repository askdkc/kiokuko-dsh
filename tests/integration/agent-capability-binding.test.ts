...
test("Agent Gateway binds an ephemeral catalog and rejects answer-time catalog replacement before mutation", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-agent-capability-binding-"),
  );
  ...
  try {
    ...
    assert.equal(
      database
        .prepare(
          "SELECT question_count AS count FROM akinator_sessions WHERE id = ?",
        )
        .get<{ count: number }>(opened.intakeSessionId)?.count,
      0,
    );
    ...
    const persisted = JSON.stringify({
      ...
      idempotency: database
        .prepare("SELECT request_hash, response_json FROM gateway_idempotency")
        .all(),
    });
