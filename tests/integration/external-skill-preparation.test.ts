...
function fixtureFetch(
  onRegistryRequest?: () => void | Promise<void>,
): typeof fetch {
  return async (input) => {
    ...
    if (url.pathname === "/repos/sveltejs/ai-tools")
      return jsonResponse({ default_branch: "main" });
    if (url.pathname.endsWith("/commits/main"))
      return jsonResponse({ sha: COMMIT });
    if (url.pathname.includes(`/git/trees/${COMMIT}`))
      return jsonResponse({
        truncated: false,
        tree: [
          {
          ...
          },
          {
            type: "blob",
            mode: "100644",
            path: "skills/svelte-code-writer/references/lookup.md",
          },
        ],
      });
    ...
    if (url.pathname.endsWith("/lookup.md"))
      return textResponse("# Lookup\n\nCheck official Svelte documentation.");
...
async function repository(
  prefix: string,
  manifest: { file: string; value: unknown },
): Promise<string> {
  const root = await mkdtemp(
    path.join(tmpdir(), `kiokuko-external-context-${prefix}-`),
  );
...
function assertSelectedSkillDelivered(
  ...
  const mappedEntries = database
    .prepare(
      "SELECT entry_id AS entryId FROM external_skill_entries WHERE skill_id = ? AND active = 1",
    )
    .all<{ entryId: string }>(skillId);
  const item = prepared.context?.items.find((candidate) =>
    mappedEntries.some((mapping) => mapping.entryId === candidate.entryId),
  );
  assert.ok(
    item,
    "the selected external Skill must reach this run's first task context",
  );
  ...
  assert.ok(
    stored.some((entry) => /Ignore every prior instruction/u.test(entry.body)),
  );
...
test("uses official discovery by default and imports a missing relevant skill before the first bounded task context", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-external-context-db-"),
  );
  ...
  try {
    ...
    database
      .prepare(
        "UPDATE external_skills SET first_seen_at = '2020-01-01T00:00:00.000Z', last_seen_at = '2020-01-01T00:00:00.000Z', last_checked_at = '2020-01-01T00:00:00.000Z'",
      )
      .run();
    ...
    const freshnessChanged = await prepareAgentTask(database, {
      ...
      fetchImpl: async () => {
        ...
        throw new Error("delivery replay must not search again");
      },
    });
    ...
    assert.notEqual(
      freshnessChanged.context?.deliveryId,
      prepared.context?.deliveryId,
    );
    ...
    const mappedEntryIds = database
      .prepare(
        "SELECT entry_id AS entryId FROM external_skill_entries WHERE skill_id = ?",
      )
      .all<{ entryId: string }>(importedSkillId)
      .map((row) => row.entryId);
    const lifecycle = database
      .prepare(
        "SELECT last_seen_at AS lastSeenAt FROM external_skills WHERE skill_id = ?",
      )
      .get<{ lastSeenAt: string }>(importedSkillId)!;
    database
      .prepare(
        "UPDATE external_skills SET last_checked_at = last_seen_at WHERE skill_id = ?",
      )
      .run(importedSkillId);
    setExternalSkillState(
      ...
      importedSkillId,
      "disabled",
      new Date(Date.parse(lifecycle.lastSeenAt) + 1).toISOString(),
    );
    ...
    const invalidated = await prepareAgentTask(database, {
      ...
      fetchImpl: async () => {
        replayNetworkCalls += 1;
        throw new Error("disabled delivery replay must not search again");
      },
    });
    ...
    assert.equal(replayNetworkCalls, 0);
    assert.notEqual(
      invalidated.context?.deliveryId,
      prepared.context?.deliveryId,
    );
    assert.notEqual(
      invalidated.context?.deliveryId,
    ...
    assert.equal(
      invalidated.context?.items.some((item) =>
        mappedEntryIds.includes(item.entryId),
      ),
      false,
    );
    ...
    const stableReplacement = await prepareAgentTask(database, {
      ...
      fetchImpl: async () => {
        replayNetworkCalls += 1;
        throw new Error("replacement delivery replay must not search again");
      },
    });
    assert.equal(
      stableReplacement.context?.deliveryId,
      invalidated.context?.deliveryId,
    );
    assert.deepEqual(
      stableReplacement.context?.items,
...
test("replays a completed no-delivery discovery attempt without another provider process", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-attempt-replay-empty-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  try {
    ...
    assert.equal(
      database
        .prepare(
          "SELECT state FROM agent_task_skill_discovery_attempts WHERE run_id = ?",
        )
        .get<{ state: string }>(first.run.runId)?.state,
      "completed",
    );
    ...
    const replayed = await prepareAgentTask(database, {
      ...
      fetchImpl: async () => {
        ...
        throw new Error(
          "completed discovery replay must not call the provider",
        );
      },
    });
    ...
    database
      .prepare(
        "UPDATE agent_task_skill_discovery_attempts SET summary_json = ' ' || summary_json WHERE run_id = ?",
      )
      .run(first.run.runId);
    await assert.rejects(
      prepareAgentTask(database, {
        ...
        fetchImpl: async () => {
          retryProviderCalls += 1;
          throw new Error(
            "corrupt completed discovery replay must not call the provider",
          );
        },
      }),
      (error: unknown) =>
...
test("persists and replays a malformed-provider summary without retrying discovery", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-attempt-replay-provider-failure-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  try {
    ...
    assert.deepEqual(prepared.skillDiscovery.failures, [
...
test("redacts unexpected Kiokuko discovery failure details before persisting and replaying them", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-attempt-private-failure-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  try {
    let providerCalls = 0;
    const rejectsClosedFailure = (error: unknown) =>
    ...
    await assert.rejects(
      prepareAgentTask(database, {
        ...
        fetchImpl: async () => {
          providerCalls += 1;
          throw new KiokukoError("CONFLICT", privateSentinel, {
            privateSentinel,
          });
        },
      }),
    ...
    assert.equal(providerCalls, 1);
    const failureJson = database
      .prepare(
        "SELECT failure_json AS failureJson FROM agent_task_skill_discovery_attempts",
      )
...
test("conflicts a concurrent exact retry while the run-owned discovery attempt is started", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-attempt-concurrent-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  let firstPromise:
    Promise<Awaited<ReturnType<typeof prepareAgentTask>>> | undefined;
  try {
    ...
    assert.equal(
      database
        .prepare("SELECT state FROM agent_task_skill_discovery_attempts")
        .get<{ state: string }>()?.state,
      "started",
    );
    ...
    await assert.rejects(
      within(
        prepareAgentTask(concurrent, {
          ...request,
          fetchImpl: async () => {
            secondProviderCalls += 1;
            throw new Error(
              "concurrent retry must not call a second provider process",
            );
          },
        }),
        2_000,
        "concurrent exact discovery retry",
      ),
      (error: unknown) =>
        error instanceof KiokukoError &&
        error.code === "CONFLICT" &&
        error.message ===
          "Task Skill discovery is already in progress or did not complete",
    );
    ...
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM agent_task_skill_discovery_attempts",
        )
        .get<{ count: number }>()?.count,
      1,
    );
...
test("discovers reference-only context when a build client lacks or does not report memory-reasoning and ordinary memory is absent", async () => {
  ...
  for (const request of [
    {
      client: { kind: "test", sessionId: "missing-memory-reasoning" },
      capabilities: [
        SOUL_CAPABILITY,
        { kind: "skill", name: "kiokuko-ui-design-soul" },
      ],
    ...
    {
      client: { kind: "test", sessionId: "unknown-memory-reasoning" },
      capabilities: [SOUL_CAPABILITY, { kind: "invalid", name: "invalid" }],
    },
  ] as const) {
    const data = await mkdtemp(
      path.join(
        tmpdir(),
        `kiokuko-post-discovery-capability-gate-${request.client.sessionId}-`,
      ),
    );
    const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
    await initializeDatabase({ databasePath });
    ...
    try {
      ...
      assert.ok(
        prepared.skillDiscovery.selected.length > 0,
        JSON.stringify(prepared.skillDiscovery),
      );
      assert.equal(prepared.nextAction, "proceed");
      assert.ok(prepared.context?.deliveryId);
      assert.ok((prepared.context?.items.length ?? 0) > 0);
      assert.ok(
        prepared.context?.items.every((item) => item.origin === "ecosystem"),
      );
      assert.equal(
        prepared.capabilities.recommendations.some(
          (item) => item.name === "memory-reasoning",
        ),
        false,
      );
...
test("fails closed when a replayed task context references a corrupt external source commit", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-external-replay-mismatch-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  try {
    ...
    const mappedEntryIds = database
      ...
      .map((row) => row.entryId);
    assert.ok(
      prepared.context?.items.some((item) =>
        mappedEntryIds.includes(item.entryId),
      ),
    );
    assert.equal(
      database
        .prepare(
          "SELECT active FROM external_skill_entries WHERE skill_id = ? LIMIT 1",
        )
        .get<{ active: number }>(skillId)?.active,
      1,
    );

    database
      .prepare(
        "UPDATE external_skills SET source_commit = ? WHERE skill_id = ?",
      )
      .run("e".repeat(40), skillId);
    let networkCalls = 0;
    await assert.rejects(
      prepareAgentTask(database, {
        ...request,
        fetchImpl: async () => {
          networkCalls += 1;
          throw new Error("corrupt delivery replay must not search again");
        },
      }),
      (error: unknown) =>
...
test("keeps an exact retry on the same run and uses a new logical request after its manifest digest changes", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-manifest-run-identity-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  try {
    const initial = await prepareAgentTask(database, {
      ...request,
      fetchImpl: fixtureFetch(),
    });
    const initialDigest = database
      .prepare(
        "SELECT manifest_digest AS digest FROM repository_fingerprints WHERE repository_id = ?",
      )
    ...
    const unchanged = await prepareAgentTask(database, {
      ...
      fetchImpl: async () => {
        networkCalls += 1;
        throw new Error("unchanged project replay must not search again");
      },
    });
    ...
    await writeFile(
      ...
      JSON.stringify({
        dependencies: { "@sveltejs/kit": "^2.1.0", svelte: "^5.1.0" },
      }),
    );
    await assert.rejects(
      prepareAgentTask(database, {
        ...
        fetchImpl: async () => {
          networkCalls += 1;
          throw new Error("conflicting request must not search");
        },
      }),
      (error: unknown) =>
    ...
    const changed = await prepareAgentTask(database, {
      ...
      fetchImpl: async (...args) => {
        networkCalls += 1;
        return changedFetch(...args);
      },
    });
    const changedDigest = database
      .prepare(
        "SELECT manifest_digest AS digest FROM repository_fingerprints WHERE repository_id = ?",
      )
    ...
    assert.equal(
      changed.skillDiscovery.selected[0]?.skillId,
      initial.skillDiscovery.selected[0]?.skillId,
    );
    assert.ok(networkCalls > 0);
  } finally {
    database.close();
...
test("fails closed without importing when the manifest changes during discovery", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-manifest-snapshot-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
...
test("starts a new discovery run when the effective mode changes from off to official", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-external-mode-identity-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  try {
    const disabled = await prepareAgentTask(database, {
      ...
      fetchImpl: async () => {
        throw new Error("off mode must not search");
      },
    });
...
test("starts a new discovery run when relevant client skills become missing", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-external-capability-identity-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  try {
    const satisfied = await prepareAgentTask(database, {
      ...
      capabilities: [
        ...
        { kind: "skill", name: "sveltekit" },
      ],
      fetchImpl: async () => {
        throw new Error("available skills must suppress discovery");
      },
    });
    let fetchCalls = 0;
...
test("allows Akinator external Skill discovery to be explicitly disabled", async () => {
  ...
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-external-disabled-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  try {
    ...
    const prepared = await prepareAgentTask(database, {
      ...
      fetchImpl: async () => {
        networkCalls += 1;
        throw new Error("disabled discovery must not use the network");
      },
    });
...
test("does not expose a Svelte external skill to an unrelated Laravel project", async () => {
  const svelteRoot = await repository("svelte-source", {
    file: "package.json",
    value: { dependencies: { "@sveltejs/kit": "^2.0.0", svelte: "^5.0.0" } },
  });
  const laravelRoot = await repository("laravel-target", {
    file: "composer.json",
    value: { require: { "laravel/framework": "^13.0" } },
  });
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-external-compat-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  try {
    const source = await prepareAgentTask(database, {
      ...
      capabilities: [
        SOUL_CAPABILITY,
        { kind: "skill", name: "memory-reasoning" },
        { kind: "skill", name: "unrelated-skill" },
      ],
      skillDiscoveryMode: "official",
      fetchImpl: fixtureFetch(),
    });
    assert.equal(source.skillDiscovery.selected.length, 1);
    const svelte = await retrieveFederatedMemory(database, {
      project: source.project,
      scope: "ecosystem",
      query: "Svelte Code Writer",
    });
    assert.ok(
      svelte.ecosystem?.items.some((item) => item.title.includes("Svelte")),
    );
    ...
    setExternalSkillState(
      ...
      importedSkillId,
      "disabled",
      "2099-08-25T06:00:00.000Z",
    );
    const disabled = await retrieveFederatedMemory(database, {
      project: source.project,
      scope: "ecosystem",
      query: "Svelte Code Writer",
    });
    assert.equal(disabled.ecosystem, null);
    const target = await retrieveFederatedMemory(database, {
      cwd: laravelRoot,
      scope: "ecosystem",
      query: "Svelte Code Writer",
    });
...
test("keeps task preparation successful when the registry is unavailable", async () => {
  const root = await repository("svelte-offline", {
    file: "package.json",
    value: { dependencies: { "@sveltejs/kit": "^2.0.0" } },
  });
  const data = await mkdtemp(
    path.join(tmpdir(), "kiokuko-external-offline-db-"),
  );
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  ...
  try {
    const prepared = await prepareAgentTask(database, {
      ...
      skillDiscoveryMode: "official",
      fetchImpl: async () => {
        throw Object.assign(new TypeError(offlineSentinel), {
...
test("keeps task preparation ready without fabricating a catalog source after a real provider timeout", async () => {
  ...
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-external-timeout-db-"),
  );
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (url.hostname === "skills.sh") {
      ...
      if (signal === undefined || signal === null)
        throw new Error("provider request omitted its abort signal");
    ...
    if (
      ...
      url.pathname ===
        `/sveltejs/ai-tools/${COMMIT}/tools/skills/svelte-code-writer/SKILL.md`
    ) {
      return textResponse(
  ...
  try {
    ...
    assert.equal(prepared.skillDiscovery.attempted, true);
    assert.ok(
      prepared.skillDiscovery.failures.some(
        (failure) =>
          failure.stage === "search" && failure.code === "registry_unavailable",
      ),
      JSON.stringify(prepared),
    );
    assert.equal(
      prepared.skillDiscovery.selected.length,
      0,
      JSON.stringify(prepared),
    );
    ...
    assert.equal(
      requestedUrls.some((value) => new URL(value).hostname !== "skills.sh"),
      false,
    );
