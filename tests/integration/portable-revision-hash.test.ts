...
import { readEntry, recordEntry, type EntryRecord, type RecordEntryInput } from '../../src/memory/entries.js';
...
const repositoryMigrations = path.resolve(import.meta.dirname, '../../migrations');
...
async function v8Database(prefix: string): Promise<V8Database> {
  ...
  for (let version = 1; version <= 8; version += 1) {
    ...
    const name = migrationFiles.find((candidate) => candidate.startsWith(prefixValue));
    assert.notEqual(name, undefined);
    await copyFile(path.join(repositoryMigrations, name as string), path.join(migrationsDirectory, name as string));
  }
  const db = openConnection(databasePath);
  assert.deepEqual(migrateDatabase(db, migrationsDirectory).applied, [1, 2, 3, 4, 5, 6, 7, 8]);
...
function migrationVersion(db: ReturnType<typeof openConnection>): number {
  return Number(db.prepare('SELECT MAX(version) AS version FROM schema_migrations').get<{ version: unknown }>()?.version);
}
...
test('migration 009 canonicalizes released hashes and scope before runtime, export, and import', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      { ...source.db.prepare('SELECT singleton, algorithm FROM entry_revision_hash_format').get() },
      { singleton: 1, algorithm: 'canonical-json-utf16-tags-v1' },
    );

    const upgraded = readEntry(source.db, { workspace: entry.workspace, entryId: entry.id });
    assert.equal(upgraded.contentHash, canonicalEntryRevisionContentHash(upgraded));
    assert.deepEqual(upgraded.tags, canonicalTagOrder(TAGS));
    assert.deepEqual(
      ((upgraded.scope.applicability as JsonObject).frameworks as Array<{ name: string }>).map(({ name }) => name),
      ['z', 'ä'],
    );
    const replay = recordEntry(source.db, input);
    assert.equal(replay.id, entry.id);
    assert.equal(source.db.prepare('SELECT COUNT(*) AS count FROM entries WHERE workspace = ?').get<{ count: number }>(entry.workspace)?.count, 1);
    ...
    assert.deepEqual(
      ((normalizedWrite.scope.applicability as JsonObject).frameworks as Array<{ name: string }>).map(({ name }) => name),
      ['z', 'ä'],
    );
    ...
    const targetDirectory = await mkdtemp(path.join(tmpdir(), 'kiokuko-canonical-revision-import-'));
    const target = openConnection(path.join(targetDirectory, 'kiokuko.sqlite3'));
    try {
      migrateDatabase(target);
      assert.equal((await importWorkspace(target, { input: archivePath })).imported, 1);
      assert.equal(exportWorkspace(target, { workspace: entry.workspace }).content, archive.content);
    } finally {
      target.close();
    }
  } finally {
...
test('migration 009 rejects an unbound legacy preimage and rolls back every schema change', async () => {
  ...
  try {
    ...
    assert.equal(source.db.prepare("SELECT 1 AS present FROM sqlite_schema WHERE name = 'entry_revision_hash_format'").get(), undefined);
    assert.equal(source.db.prepare("SELECT 1 AS present FROM sqlite_schema WHERE name = 'agent_task_skill_discovery_attempts'").get(), undefined);
    assert.equal(source.db.prepare('SELECT content_hash FROM entry_revisions WHERE entry_id = ?').get<{ content_hash: string }>(entry.id)?.content_hash, legacyHash);
    assert.notEqual(source.db.prepare("SELECT sql FROM sqlite_schema WHERE type = 'trigger' AND name = 'entry_revisions_immutable_update'").get(), undefined);
  } finally {
    source.db.close();
  }
});
...
test('migration 009 rejects canonical identity collisions without partially rewriting rows', async () => {
  ...
  try {
    ...
    assert.equal(source.db.prepare("SELECT 1 AS present FROM sqlite_schema WHERE name = 'agent_task_skill_discovery_attempts'").get(), undefined);
    assert.deepEqual(
      source.db.prepare('SELECT entry_id, content_hash FROM entry_revisions ORDER BY entry_id').all().map((row) => ({ ...row })),
      [
