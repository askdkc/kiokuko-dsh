...
async function database(prefix: string) {
  const directory = await mkdtemp(
    path.join(tmpdir(), `kiokuko-execution-context-db-${prefix}-`),
  );
...
test("task preparation exposes the launch repository as the canonical absolute path base", async () => {
  const root = await mkdtemp(
    path.join(tmpdir(), "kiokuko-execution-context-root-"),
  );
  ...
  try {
    ...
    assert.equal(
      ...
      prepared.executionContext.repositoryRoot,
    );
    assert.match(
      prepared.securityNotice,
      /Use executionContext\.repositoryRoot as the canonical base/u,
    );
  } finally {
    connection.close();
  }
...
test("task preparation distinguishes a nested cwd while retaining the canonical repository root", async () => {
  const root = await mkdtemp(
    path.join(tmpdir(), "kiokuko-execution-context-nested-"),
  );
  execFileSync("git", ["init", "-q", root]);
...
test("task preparation treats a Git worktree with a .git file as its canonical repository root", async () => {
  const main = await mkdtemp(
    path.join(tmpdir(), "kiokuko-execution-context-main-"),
  );
  ...
  const worktreeParent = await mkdtemp(
    path.join(tmpdir(), "kiokuko-execution-context-worktree-parent-"),
  );
