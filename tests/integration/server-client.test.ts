...
test("write request snapshots JSON and sends the exact idempotency and content headers", async () => {
  ...
  try {
    ...
    assert.equal(
      receivedBody,
      '{"nested":{"a":"before-mutation","b":2},"z":"snapshot"}',
    );
  } finally {
    await close(local.server);
  }
...
test("canonical request snapshots preserve __proto__ as data and never mutate object prototypes", async () => {
  ...
  const client = await clientForFetch(
    "client-proto-body",
    async (_url, init) => {
  ...
  const body = JSON.parse(
    '{"ordinary":1,"__proto__":{"polluted":true}}',
...
test(
  "request snapshots never invoke ambient Array.prototype.toJSON hooks",
  { concurrency: false },
  async () => {
    let callbackCalls = 0;
    let receivedBody: string | undefined;
    ...
    try {
      const client = await clientForFetch(
        "client-array-to-json",
        async (_url, init) => {
          receivedBody = typeof init?.body === "string" ? init.body : undefined;
          return jsonResponse({
            apiVersion: "1",
            ok: true,
            operation: "agent.array",
            data: { accepted: true },
          });
        },
      );
      assert.deepEqual(
        await client.request({
          method: "POST",
          path: "/api/v1/agent/array",
          operation: "agent.array",
          body: { items: [1] },
          idempotencyKey: "array-key",
        }),
        { accepted: true },
      );
...
test("request and body boundaries reject proxies, accessors, hidden fields, and non-plain shapes without invoking them", async () => {
  ...
  const nonPlainBody = Object.create({ inherited: true }) as Record<
    string,
    unknown
  >;
...
test("revoked request and body proxies fail as validation errors without proxy introspection", async () => {
  ...
  await assert.rejects(
    () =>
      client.request({
        method: "POST",
        path: "/api/v1/agent/revoked",
        operation: "agent.revoked",
        body: revokedBody.proxy,
        idempotencyKey: "revoked-body",
      }),
    (error: unknown) =>
      error instanceof Error &&
      "code" in error &&
    ...
    await assert.rejects(
      () =>
        client.request({
          ...
          ...(input.method === "POST" ? { idempotencyKey: "path-test" } : {}),
        }),
      (error: unknown) => {
        assert.equal(
          error instanceof Error &&
...
test("rejects non-JSON, cyclic, non-finite, and oversized bodies without echoing input", async () => {
  ...
  Object.defineProperty(malformedArray, "01", {
    value: "non-canonical-index",
    enumerable: true,
  });
...
test("rejects malformed, empty, HTML, oversized, and wrong-operation responses as integrity errors", async () => {
  const responses = [
    new Response("<html>response-secret</html>", {
      status: 200,
    ...
    jsonResponse(
      { apiVersion: "1", ok: true, operation: "agent.response", data: {} },
      201,
    ),
  ];
  ...
  for (const [index, response] of responses.entries()) {
    const client = await clientForFetch(
      `client-response-integrity-${index}`,
      async () => response,
    );
    await assert.rejects(
      () =>
      ...
      (error: unknown) => {
        ...
        assert.equal(
          error instanceof Error && error.message.includes("response-secret"),
          false,
        );
        return true;
      },
  ...
  const oversizedClient = await clientForFetch(
    "client-response-oversized",
    async () => oversized,
  );
  await assert.rejects(
    () =>
      oversizedClient.request({
        method: "GET",
        path: "/api/v1/agent/response",
        operation: "agent.response",
      }),
...
test("rejects non-canonical response JSON before duplicate envelope fields can override each other", async () => {
  ...
  for (const [index, body] of bodies.entries()) {
    const client = await clientForFetch(
      `client-response-strict-json-${index}`,
      async () =>
        new Response(body, {
          status: 200,
          headers: { "content-type": "application/json" },
...
test("response streaming accepts exact byte chunks and rejects decorated or derived chunks", async () => {
  const encoded = new TextEncoder().encode(
    '{"apiVersion":"1","ok":true,"operation":"agent.bytes","data":{"accepted":true}}',
  );
  const exactChunkClient = await clientForFetch(
    "client-exact-byte-chunk",
    async () =>
  ...
  assert.deepEqual(
    await exactChunkClient.request({
      method: "GET",
      path: "/api/v1/agent/bytes",
      operation: "agent.bytes",
    }),
    { accepted: true },
  );
  ...
  for (const chunk of [
    ...
    new (class extends Uint8Array {})(encoded),
  ]) {
    const client = await clientForFetch(
      "client-invalid-byte-chunk",
      async () =>
        new Response(
          new ReadableStream({
            start(controller) {
...
test("rejects pre-consumed and locked exact responses before parsing a suffix", async () => {
  ...
  const disturbed = new Response(
    new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        controller.enqueue(
          encoder.encode(
            '{"apiVersion":"1","ok":true,"operation":"agent.disturbed","data":{"accepted":true}}',
          ),
        );
        controller.close();
      },
  ...
  const disturbedClient = await clientForFetch(
    "client-disturbed-response",
    async () => disturbed,
  );
  await assert.rejects(
    () =>
      disturbedClient.request({
        method: "GET",
        path: "/api/v1/agent/disturbed",
        operation: "agent.disturbed",
      }),
    (error: unknown) =>
      error instanceof Error &&
      "code" in error &&
  ...
  const locked = jsonResponse({
    apiVersion: "1",
    ok: true,
    operation: "agent.locked",
    data: { accepted: true },
  });
  const lockedReader = locked.body?.getReader();
  try {
    const lockedClient = await clientForFetch(
      "client-locked-response",
      async () => locked,
    );
    await assert.rejects(
      () =>
        lockedClient.request({
          method: "GET",
          path: "/api/v1/agent/locked",
          operation: "agent.locked",
        }),
      (error: unknown) =>
        error instanceof Error &&
...
test("bounds zero-byte response chunk floods", async () => {
  ...
  const floodClient = await clientForFetch(
    "client-zero-chunk-flood",
    async () => flood,
  );
  await assert.rejects(
    () =>
      floodClient.request({
        method: "GET",
        path: "/api/v1/agent/flood",
        operation: "agent.flood",
      }),
    (error: unknown) =>
      error instanceof Error &&
      "code" in error &&
...
test("preserves response read and cleanup failures in causal order", async () => {
  const request = {
    method: "GET" as const,
    path: "/api/v1/agent/cleanup",
    operation: "agent.cleanup",
  };
  const failedBody = (failure: Error) =>
    new Response(
      new ReadableStream<Uint8Array>({
        pull(controller) {
  ...
  const singleCleanupClient = await clientForFetch(
    "client-single-cleanup-failure",
    async () => failedBody(readFailure),
  );
  await assert.rejects(
    singleCleanupClient.request(request),
    (error: unknown) => {
      ...
      assert.equal(
        failures[1] instanceof Error &&
          "code" in failures[1] &&
          failures[1].code === "INTEGRITY_ERROR",
        true,
      );
      return true;
    },
  ...
  try {
    ...
    const combinedClient = await clientForFetch(
      "client-combined-cleanup-failure",
      async () => failedBody(combinedReadFailure),
    );
    ...
    const valid = jsonResponse({
      apiVersion: "1",
      ok: true,
      operation: "agent.cleanup",
      data: { accepted: true },
    });
    const releaseOnlyClient = await clientForFetch(
      "client-release-only-failure",
      async () => valid,
    );
    await assert.rejects(
      releaseOnlyClient.request(request),
      (error: unknown) => {
    ...
    await assert.rejects(
      () =>
        client.request({
          ...
          operation: "agent.error",
          ...(input.code === "CONFLICT"
            ? { idempotencyKey: "known-error-key" }
            : {}),
        }),
      (error: unknown) => {
        assert.equal(
        ...
        const details =
          ...
          !Array.isArray(error.details)
            ...
            : {};
        assert.equal(
          JSON.stringify(details).includes("known-error-key"),
          false,
        );
        if (input.code === "BACKPRESSURE")
          assert.equal(details.retryAfterSeconds, 7);
        return true;
      },
    );
...
test("rejects HTTP status, error code, and Retry-After contradictions", async () => {
    ...
    jsonResponse(
      {
        ...
        ok: false,
        operation: "agent.error",
        error: { code: "BACKPRESSURE", message: "Busy", details: {} },
      },
      429,
    ),
    jsonResponse(
    ...
    jsonResponse(
      {
        ...
        ok: false,
        operation: "agent.error",
        error: { code: "PARTIAL_FAILURE", message: "Partial", details: {} },
      },
      500,
    ),
  ];
  ...
  for (const [index, response] of responses.entries()) {
    const client = await clientForFetch(
      `client-contradictory-error-${index}`,
      async () => response,
    );
    await assert.rejects(
      () =>
    ...
    jsonResponse(
      {
        ...
        ok: false,
        operation: "agent.error",
        error: { code: "UNKNOWN_CODE", message: "unknown", details: {} },
      },
      500,
    ),
    jsonResponse(
      {
        ...
        ok: false,
        operation: "agent.error",
        error: { code: "CONFLICT", message: "unsafe-error-key", details: {} },
      },
      409,
    ),
    jsonResponse(
    ...
    jsonResponse(
      {
        ...
        error: {
          code: "CONFLICT",
          message: "unsafe",
          details: { authorization: "header-secret" },
        },
      },
      409,
    ),
    jsonResponse(
      {
        ...
        error: {
          code: "CONFLICT",
          message: "unsafe",
          details: { body: "request-body-secret" },
        },
      },
      409,
    ),
    ...
    await assert.rejects(
      () =>
        client.request({
          ...
          path: "/api/v1/agent/error",
          operation: "agent.error",
          ...(index === 1 ? { idempotencyKey: "unsafe-error-key" } : {}),
        }),
      (error: unknown) => {
        assert.equal(
          error instanceof Error &&
...
test("rejects server errors that echo a request-body value", async () => {
  const client = await clientForFetch("client-error-body-echo", async () =>
    jsonResponse(
      {
        apiVersion: "1",
        ok: false,
...
test("maps only exact network failures and propagates programming errors and caller aborts without retrying", async () => {
  ...
  const networkClient = await clientForFetch(
  ...
  await assert.rejects(
    () =>
      networkClient.request({
        method: "POST",
        path: "/api/v1/agent/network",
        operation: "agent.network",
        body: { value: "request-secret" },
    ...
    (error: unknown) => {
      ...
      assert.equal(
        error instanceof Error && error.message.includes("network-write-key"),
        false,
      );
      assert.equal(
        error instanceof Error && error.message.includes("127.0.0.1"),
        false,
      );
      return true;
    },
  ...
  const codedProgrammingSentinel = Object.assign(
    new Error("not a native fetch failure"),
    { code: "ECONNREFUSED" },
  );
  const codedProgrammingClient = await clientForFetch(
    "client-coded-programming-failure",
    async () => {
  ...
  await assert.rejects(
    () =>
      codedProgrammingClient.request({
        method: "GET",
        path: "/api/v1/agent/coded-programming",
        operation: "agent.coded-programming",
      }),
    (error: unknown) => error === codedProgrammingSentinel,
  );

  const plainCauseSentinel = new TypeError("fetch failed", {
    cause: { code: "ECONNREFUSED" },
  });
  const plainCauseClient = await clientForFetch(
    "client-plain-network-cause",
    async () => {
  ...
  await assert.rejects(
    () =>
      plainCauseClient.request({
        method: "GET",
        path: "/api/v1/agent/plain-cause",
        operation: "agent.plain-cause",
      }),
    (error: unknown) => error === plainCauseSentinel,
  );

  const extendedNetworkFailure = Object.assign(
    new TypeError("fetch failed", { cause: networkCause }),
    { extra: true },
  );
  const extendedNetworkClient = await clientForFetch(
    "client-extended-network-failure",
    async () => {
  ...
  await assert.rejects(
    () =>
      extendedNetworkClient.request({
        method: "GET",
        path: "/api/v1/agent/extended-network",
        operation: "agent.extended-network",
      }),
  ...
  const programmingClient = await clientForFetch(
    "client-programming-failure",
    async () => {
      programmingCalls += 1;
  ...
  await assert.rejects(
    () =>
      programmingClient.request({
        method: "GET",
        path: "/api/v1/agent/programming",
        operation: "agent.programming",
      }),
  ...
  let abortCalls = 0;
  const abortSentinel = Object.assign(new Error("caller aborted"), {
    code: "ECONNRESET",
  });
  ...
  await assert.rejects(
    () =>
      preAbortClient.request({
        method: "GET",
        path: "/api/v1/agent/pre-abort",
        operation: "agent.pre-abort",
  ...
  const resolvedAbortClient = await clientForFetch(
    "client-resolved-abort",
    async () => {
      resolvedAbort.abort(resolvedAbortSentinel);
      return jsonResponse({
        apiVersion: "1",
        ok: true,
        operation: "agent.abort-resolved",
        data: { accepted: true },
      });
    },
  );
  await assert.rejects(
    () =>
      resolvedAbortClient.request({
        method: "GET",
        path: "/api/v1/agent/abort-resolved",
        operation: "agent.abort-resolved",
        signal: resolvedAbort.signal,
      }),
    (error: unknown) => error === resolvedAbortSentinel,
  );

  const invalidReturn = { status: 200 };
  const invalidReturnClient = await clientForFetch(
    "client-non-response",
    async () => invalidReturn as never,
  );
  await assert.rejects(
    () =>
      invalidReturnClient.request({
        method: "GET",
        path: "/api/v1/agent/non-response",
        operation: "agent.non-response",
      }),
    (error: unknown) =>
      error instanceof TypeError &&
      error.message === "Fetch implementation returned a non-Response value",
  );
  ...
  const proxiedErrorClient = await clientForFetch(
    "client-proxied-error",
    async () => {
      throw proxiedError;
    },
  );
  await assert.rejects(
    () =>
      proxiedErrorClient.request({
        method: "GET",
        path: "/api/v1/agent/proxied-error",
        operation: "agent.proxied-error",
      }),
  ...
  const proxiedResponse = new Proxy(
    jsonResponse({
      apiVersion: "1",
      ok: true,
      operation: "agent.proxied-response",
      data: { accepted: true },
    }),
    {
      getPrototypeOf() {
  ...
  const proxiedResponseClient = await clientForFetch(
    "client-proxied-response",
    async () => proxiedResponse,
  );
  await assert.rejects(
    () =>
      proxiedResponseClient.request({
        method: "GET",
        path: "/api/v1/agent/proxied-response",
        operation: "agent.proxied-response",
      }),
...
test("caller abort interrupts non-settling fetches and cancels pending response readers", async () => {
  ...
  const pendingFetchClient = await clientForFetch(
    "client-pending-fetch-abort",
    async () => new Promise<Response>(() => {}),
  );
  const pendingFetch = pendingFetchClient.request({
    method: "GET",
    path: "/api/v1/agent/pending-fetch",
    operation: "agent.pending-fetch",
  ...
  const pendingReaderClient = await clientForFetch(
    "client-pending-reader-abort",
    async () => pendingBody,
  );
  const pendingReader = pendingReaderClient.request({
    method: "GET",
    path: "/api/v1/agent/pending-reader",
    operation: "agent.pending-reader",
  ...
  await assert.rejects(
    pendingReader,
    (error: unknown) => error === readerReason,
  );
  assert.equal(cancelCalls, 1);
  ...
  const failedCancelPending = new Promise<void>((resolve) => {
    markFailedCancelPending = resolve;
  });
  const failedCancelBody = new Response(
    new ReadableStream<Uint8Array>(
      {
        pull() {
  ...
  const failedCancelClient = await clientForFetch(
    "client-failed-reader-cancel",
    async () => failedCancelBody,
  );
  const failedCancelRequest = failedCancelClient.request({
    method: "GET",
    path: "/api/v1/agent/failed-cancel",
    operation: "agent.failed-cancel",
  ...
  const hungCancelPending = new Promise<void>((resolve) => {
    markHungCancelPending = resolve;
  });
  const hungCancelBody = new Response(
    new ReadableStream<Uint8Array>(
      {
        pull() {
  ...
  const hungCancelClient = await clientForFetch(
    "client-hung-reader-cancel",
    async () => hungCancelBody,
  );
  const hungCancelRequest = hungCancelClient.request({
    method: "GET",
    path: "/api/v1/agent/hung-cancel",
    operation: "agent.hung-cancel",
...
test("maps only the exact native mid-body transport signature", async () => {
  const local = await listen((_request, response) => {
    ...
    response.write(
      '{"apiVersion":"1","ok":true,"operation":"agent.partial","data":',
    );
  ...
  try {
    ...
    await assert.rejects(
      () =>
        client.request({
          method: "GET",
          path: "/api/v1/agent/partial",
          operation: "agent.partial",
        }),
      (error: unknown) => {
        assert.equal(error instanceof AggregateError, true);
        const failures = error instanceof AggregateError ? error.errors : [];
        ...
        assert.equal(
          failures[0] instanceof Error &&
            "code" in failures[0] &&
            failures[0].code === "SERVICE_UNAVAILABLE",
          true,
        );
        assert.equal(
          failures[1] instanceof Error &&
  ...
  const sentinel = new TypeError("terminated", {
    cause: { code: "UND_ERR_SOCKET" },
  });
  const sentinelClient = await clientForFetch(
    "client-fake-terminated",
    async () => {
      throw sentinel;
    },
  );
  await assert.rejects(
    () =>
      sentinelClient.request({
        method: "GET",
        path: "/api/v1/agent/fake-terminated",
        operation: "agent.fake-terminated",
      }),
...
test("allows bounded read queries and enforces the exact health response contract", async () => {
  ...
  assert.equal(
    requestedUrls[0],
    "http://127.0.0.1:49152/api/v1/agent/list?cursor=opaque-cursor&limit=10",
  );

  const health = await client.request<{ ok: true }>({
    method: "GET",
    path: "/health/ready",
    operation: "health.ready",
  });
...
test("client foundation has no SQLite import or direct-service fallback", async () => {
  const source = await readFile(
    new URL("../../src/client/server-client.ts", import.meta.url),
    "utf8",
  );
  assert.equal(
    /runtime-discovery|node:sqlite|better-sqlite|sqlite3|openConnection|directService|direct-service/i.test(
      source,
    ),
    false,
  );
