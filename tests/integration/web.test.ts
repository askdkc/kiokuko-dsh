...
test("web filters by workspace, type, and cross-genre tags and supports candidate editing", async () => {
  ...
  try {
    const workspaces = (await webFetch(web.url, "/api/workspaces").then(
      (response) => response.json(),
    )) as { workspaces: Array<{ workspace: string }> };
    assert.deepEqual(
      workspaces.workspaces.map((item) => item.workspace),
      ["project:web-test"],
    );

    const filtered = (await webFetch(
      web.url,
      "/api/entries?workspace=project%3Aweb-test&kind=decision",
    ...
    const crossGenre = (await webFetch(
      web.url,
      "/api/entries?workspace=project%3Aweb-test&tag=web",
    ).then((response) => response.json())) as {
      entries: Array<{ id: string; kind: string }>;
    };
    assert.deepEqual(
      crossGenre.entries.map((entry) => entry.id),
      [data.candidate.id],
    );

    const tags = (await webFetch(
      web.url,
      "/api/tags?workspace=project%3Aweb-test",
    ).then((response) => response.json())) as {
    ...
    const update = await webFetch(
      web.url,
      `/api/entries/${data.candidate.id}?workspace=project%3Aweb-test`,
      {
        method: "PUT",
    ...
    const updated = (await update.json()) as {
      entry: { title: string; body: string; revision: number; tags: string[] };
    };
...
test("web memory recall returns federated origin and selection metadata", async () => {
  ...
  try {
    const response = await webFetch(
      web.url,
      "/api/memory/recall?workspace=project%3Aweb-test&q=web",
    );
    ...
    assert.ok(
      body.combined?.items[0]?.selectionReasons.includes("project_origin"),
    );
  } finally {
    await web.close();
  }
...
test("legacy Web delegates shared health and v1 paths before legacy OPTIONS handling", async () => {
  ...
  try {
    ...
    const preflight = await fetch(`${web.url}/api/v1/unknown`, {
      method: "OPTIONS",
    });
    ...
    const legacyOptions = await fetch(`${web.url}/api/workspaces`, {
      method: "OPTIONS",
    });
...
test("legacy Web mounts the same known Agent v1 route before legacy handling", async () => {
  ...
  try {
    ...
    assert.ok(
      body.data.capabilities.recommendations.some(
        (item) =>
...
test("legacy mutations admitted before close drain through the shared queue and new mutations are rejected", async () => {
  ...
  try {
    ...
    assert.equal(
      Number.parseInt(/^HTTP\/\d\.\d (\d+)/.exec(responseText)?.[1] ?? "", 10),
      200,
    );
...
test("web refuses direct edits to verified entries", async () => {
  ...
  try {
    const response = await webFetch(
      web.url,
      `/api/entries/${data.verified.id}?workspace=project%3Aweb-test`,
      {
        method: "PUT",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
