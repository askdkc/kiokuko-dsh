...
import {
  federatedEntries,
  retrieveFederatedMemory,
} from "../../src/memory/federated-retrieval.js";
...
async function repository(
  ...
  const root = await mkdtemp(
    path.join(tmpdir(), `kiokuko-federated-${prefix}-`),
  );
  execFileSync("git", ["init", "-q", root]);
  if (Object.keys(manifest).length > 0)
    await writeFile(
      path.join(root, manifest.file as string),
...
test("retrieves applicable project-owned memory through ecosystem scope without leaking local paths", async () => {
  ...
  const firstRoot = await repository("laravel-a", {
    file: "composer.json",
    value: { require: { "laravel/framework": "^13.0" } },
  });
  const secondRoot = await repository("laravel-b", {
    file: "composer.json",
    value: { require: { "laravel/framework": "^13.0" } },
  });
  const svelteRoot = await repository("svelte", {
    file: "package.json",
    value: { dependencies: { svelte: "^5.0", typescript: "^5.0" } },
  });
  const incompatibleRoot = await repository("laravel-11", {
    file: "composer.json",
    value: { require: { "laravel/framework": "^11.0" } },
  });
  ...
  try {
    ...
    recordEntry(database, {
      ...
      scope: buildStructuredScope({
        ...
        applicability: {
          frameworks: [{ name: "Laravel", version: ">=12 <14" }],
        },
    ...
    const initial = await recallScopedMemory(database, {
      cwd: secondRoot,
      query: "Laravel migration",
    });
    assert.equal(
      initial.ecosystem?.items.some(
        (item) => item.title === "Laravel migration rollback pattern",
      ),
      true,
    );
    const shared = initial.ecosystem?.items.find(
      (item) => item.title === "Laravel migration rollback pattern",
    );
    ...
    assert.equal(
      initial.ecosystem?.items.some(
        (item) => item.title === "Laravel private path only",
      ),
      false,
    );
    ...
    recordEntry(database, {
      ...
      scope: buildStructuredScope({
        ...
        signals: { packages: ["laravel/framework"] },
      }),
      tags: ["external:skill", "Laravel"],
    });
    const taggedOrdinary = await recallScopedMemory(database, {
      cwd: secondRoot,
      query: "Laravel migration laravel/framework",
    });
    assert.equal(
      taggedOrdinary.ecosystem?.items.some(
        (item) => item.title === "Ordinary memory with an external skill tag",
      ),
      true,
    );
    ...
    const fromLaravel = await recallScopedMemory(database, {
      cwd: secondRoot,
      query: "Laravel migration",
    });
    assert.equal(
      fromLaravel.ecosystem?.items.filter(
        (item) => item.sourceWorkspace === first.workspace,
      ).length,
      3,
    );

    const fromSvelte = await recallScopedMemory(database, {
      cwd: svelteRoot,
      query: "Laravel migration",
    });
    ...
    const signalOnlyFromSvelte = await recallScopedMemory(database, {
      cwd: svelteRoot,
      query: "sqlstate[23505]",
    });
    assert.equal(
      signalOnlyFromSvelte.ecosystem?.items.some(
        (item) => item.title === "Laravel SQLSTATE recovery",
      ) ?? false,
      false,
    );

    const fromIncompatible = await recallScopedMemory(database, {
    ...
    const prioritized = await recallScopedMemory(database, {
      cwd: secondRoot,
      query: "Laravel migration",
    });
...
test("preserves ecosystem relevance order when composing auto recall", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-federated-ranking-db-"),
  );
  const databasePath = path.join(directory, "kiokuko-dsh.sqlite3");
  const sourceRoot = await repository("ranking-source", {
    file: "composer.json",
    value: { require: { "laravel/framework": "^13.0" } },
  });
  const targetRoot = await repository("ranking-target", {
    file: "composer.json",
    value: { require: { "laravel/framework": "^13.0" } },
  });
  await initializeDatabase({ databasePath });
  ...
  try {
    ...
    assert.deepEqual(
      recalled.ecosystem?.items.map((item) => item.id),
      ["z-high-relevance", "a-low-relevance"],
    );
    assert.deepEqual(
      recalled.combined?.items.map((item) => item.id),
      ["z-high-relevance", "a-low-relevance"],
    );
  } finally {
    database.close();
...
test("keeps path-bearing tags project-local for automatic and explicit ecosystem memory", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-federated-path-tags-db-"),
  );
  const databasePath = path.join(directory, "kiokuko-dsh.sqlite3");
  const sourceRoot = await repository("path-tags-source", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  const targetRoot = await repository("path-tags-target", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  await initializeDatabase({ databasePath });
  ...
  try {
    ...
    const automatic = readEntry(database, {
      workspace: source.workspace,
      entryId: checkpoint.entries[0]!.id,
    });
    assert.equal(
      (automatic.scope as Record<string, unknown>).retrievalScope,
      undefined,
    );

    const crossProject = await recallScopedMemory(database, {
      cwd: targetRoot,
      query: "private workflow boundary",
      limit: 20,
    });
    assert.equal(
      crossProject.ecosystem?.items.some((item) => item.id === explicit.id) ??
        false,
      false,
    );
    assert.equal(
      crossProject.ecosystem?.items.some((item) => item.id === automatic.id) ??
        false,
      false,
    );
    assert.equal(
      crossProject.ecosystem?.items.some(
        (item) => item.id === projectOnly.id,
      ) ?? false,
      false,
    );

    const sameProject = await recallScopedMemory(database, {
      cwd: sourceRoot,
      query: "private workflow boundary",
      scope: "project",
      limit: 20,
    ...
    assert.equal(
      sameProject.project?.memory.items.some((item) => item.id === explicit.id),
      true,
    );
    assert.equal(
      sameProject.project?.memory.items.some(
...
test("keeps released project v2 and unversioned structured-looking scope out of ecosystem retrieval", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-federated-legacy-scope-db-"),
  );
  const databasePath = path.join(directory, "kiokuko-dsh.sqlite3");
  const sourceRoot = await repository("legacy-scope-source", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  const targetRoot = await repository("legacy-scope-target", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  await initializeDatabase({ databasePath });
  ...
  try {
    ...
    const crossProject = await recallScopedMemory(database, {
      cwd: targetRoot,
      query: "typescript workflow",
      limit: 20,
    });
    assert.equal(
      crossProject.ecosystem?.items.some((item) => item.id === v2.id) ?? false,
      false,
    );
    assert.equal(
      crossProject.ecosystem?.items.some(
        (item) => item.id === unversioned.id,
      ) ?? false,
      false,
    );
    ...
    assert.equal(
      sameProject.project?.memory.items.some((item) => item.id === v2.id),
      true,
    );
    assert.equal(
      sameProject.project?.memory.items.some(
...
test("requires an explicit v3 retrieval scope for ecosystem and global federation", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-federated-explicit-scope-db-"),
  );
  const databasePath = path.join(directory, "kiokuko-dsh.sqlite3");
  const sourceRoot = await repository("explicit-scope-source", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  const targetRoot = await repository("explicit-scope-target", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  await initializeDatabase({ databasePath });
  ...
  try {
    ...
    const implicitGlobal = recordEntry(
      database,
      {
    ...
    const explicitGlobal = recordEntry(
      database,
      {
        workspace: GLOBAL_WORKSPACE,
        kind: "lesson",
    ...
    assert.equal(
      crossProject.ecosystem?.items.some((item) => item.id === ecosystem.id) ??
        false,
      true,
    );
    assert.equal(
      crossProject.global?.items.some(
        (item) => item.id === implicitGlobal.id,
      ) ?? false,
      false,
    );
    assert.equal(
      crossProject.global?.items.some(
        (item) => item.id === explicitGlobal.id,
      ) ?? false,
      true,
    );
    assert.equal(
      crossProject.combined?.items.some((item) => item.id === projectOnly.id) ??
        false,
      false,
    );
    assert.equal(
      crossProject.combined?.items.some(
        (item) => item.id === implicitGlobal.id,
      ) ?? false,
      false,
    );
    ...
    assert.equal(
      brokerCandidates.some((item) => item.entry.id === projectOnly.id),
      false,
    );
    assert.equal(
      brokerCandidates.some(
        (item) => item.entry.id === ecosystem.id && item.origin === "ecosystem",
      ),
      true,
    );
    assert.equal(
      brokerCandidates.some((item) => item.entry.id === implicitGlobal.id),
      false,
    );
    assert.equal(
      brokerCandidates.some(
        (item) =>
          item.entry.id === explicitGlobal.id && item.origin === "global",
      ),
      true,
    );
    ...
    assert.equal(
      globalOnly.global?.items.some((item) => item.id === implicitGlobal.id) ??
        false,
      false,
    );
    assert.equal(
      globalOnly.global?.items.some((item) => item.id === explicitGlobal.id) ??
        false,
      true,
    );
...
test("preserves hybrid relevance and lane reasons before limits without status or origin bonuses", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-federated-hybrid-rank-db-"),
  );
  const databasePath = path.join(directory, "kiokuko-dsh.sqlite3");
  const root = await repository("hybrid-rank", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  await initializeDatabase({ databasePath });
  ...
  try {
    ...
    recordEntry(
      database,
      {
        ...
        scope: buildStructuredScope({
          visibility: "project",
          signals: { paths: ["src/exact.ts"] },
        }),
      },
      { idFactory: () => "z-exact-candidate" },
    );

    const first = rankedEntryHits(database, {
      workspace: project.workspace,
      query: "src/exact.ts",
      limit: 10,
    });
    const second = rankedEntryHits(database, {
      workspace: project.workspace,
      query: "src/exact.ts",
      limit: 10,
    });
    ...
    const federated = await federatedEntries(database, {
      project,
      query: "src/exact.ts",
      limit: 10,
    });
    assert.deepEqual(
      ...
      first.hits.map((hit) => hit.entryId),
    );
    ...
    const scoped = await recallScopedMemory(database, {
      cwd: root,
      query: "src/exact.ts",
      limit: 1,
    });
    ...
    assert.ok(
      scoped.combined?.items[0]?.selectionReasons.includes(
        "exact_signal_match",
      ),
    );
  } finally {
...
test("rejects invalid federated recall bounds instead of replacing zero with defaults", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-federated-bounds-db-"),
  );
  const databasePath = path.join(directory, "kiokuko-dsh.sqlite3");
  await initializeDatabase({ databasePath });
  const database = openConnection(databasePath);
  try {
    for (const limit of [0, -1, 1.5, Number.NaN, 101]) {
      await assert.rejects(
        retrieveFederatedMemory(database, {
          scope: "global",
          query: "boundary",
          limit,
        }),
...
test("counts Unicode code points across local and ecosystem recall without splitting astral characters", async () => {
  const directory = await mkdtemp(
    path.join(tmpdir(), "kiokuko-federated-unicode-db-"),
  );
  const databasePath = path.join(directory, "kiokuko-dsh.sqlite3");
  const sourceRoot = await repository("unicode-source", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  const targetRoot = await repository("unicode-target", {
    file: "package.json",
    value: { devDependencies: { typescript: "^5.9" } },
  });
  await initializeDatabase({ databasePath });
