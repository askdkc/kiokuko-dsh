...
test("use creates binding and AGENTS.md, then is unchanged on repeat", async () => {
  ...
  assert.equal(
    await readFile(path.join(root, ".kiokuko.json"), "utf8"),
    bindingBefore,
  );
  assert.equal(
...
test("new-binding ignore failure restores the binding and leaves user gitignore bytes unchanged", async () => {
  ...
  await assert.rejects(
    useRepository(
      { root, databasePath, ensureNewBindingIgnored: true },
      {
        atomicWriteTextIfUnchanged: async (
          ...
          if (path.basename(filePath) === ".gitignore")
            throw new Error("injected gitignore failure");
...
test("use adopts one exact concurrent binding and converges on an exact agent-file result", async () => {
  ...
  const result = await useRepository(
    { root, databasePath },
    {
      atomicWriteTextIfUnchanged: async (
        ...
        if (mode === undefined)
          assert.fail("use must provide an explicit file mode");
  ...
  try {
    assert.equal(
      database
        .prepare("SELECT repository_id AS repositoryId FROM repositories")
        .get<{ repositoryId: string }>()?.repositoryId,
      "repo_concurrent_winner",
    );
    assert.equal(
      database
      ...
      readAgentFileForConvergence: async (filePath, options) => {
        ...
        if (!injected && path.basename(filePath) === "AGENTS.md") {
          ...
          if (snapshot === undefined)
            assert.fail("planned agent file is missing");
...
test("use waits for a concurrent binding to shed its temporary hard link before adoption", async () => {
    ...
    { root, databasePath },
    {
      atomicWriteTextIfUnchanged: async (
        ...
        if (!injected && path.basename(filePath) === ".kiokuko.json") {
          ...
          peerWrite = atomicWriteTextIfUnchanged(
            filePath,
            winner,
            expectation,
            mode,
          ...
          try {
            return await atomicWriteTextIfUnchanged(
              filePath,
              content,
              expectation,
              mode,
  ...
  assert.equal(injected, true);
  if (peerWrite === undefined)
    assert.fail("concurrent binding writer did not start");
  await peerWrite;
    ...
    { root, databasePath },
    {
      readBindingFileForConvergence: async (filePath, options) => {
        ...
        if (!injected) {
          ...
          const plannedAgent = await readRegularFile(agentPath, {
            containmentRoot: root,
          });
          if (plannedAgent === undefined)
            assert.fail("planned agent file is missing");
...
test("use never adopts an existing binding with retained quarantine cleanup", async () => {
  ...
  const planned = await readRegularFile(bindingPath, {
  ...
  assert.equal((await stat(bindingPath)).nlink, 1);
  const retained = (await readdir(initial.repositoryRoot)).filter(
    (name) => name.startsWith("...kiokuko.json.") && name.endsWith(".cleanup"),
  );
    ...
    useRepository(
      { root, databasePath },
      {
        registerRepositoryAndLocation: () => {
          registrationCalls += 1;
          throw new Error(
            "registration must not run with retained binding cleanup",
          );
        },
      },
    ),
  ...
  if (retainedName === undefined)
    assert.fail("retained binding cleanup artifact is missing");
...
test("use waits for an identical concurrent atomic agent write to leave its quarantine window", async () => {
  ...
  let observationRaceInjected = false;
  let concurrentWrite:
    ReturnType<typeof atomicWriteTextIfUnchanged> | undefined;
  ...
  const result = await useRepository(
    ...
    },
    {
      atomicWriteTextIfUnchanged: async (
        ...
        if (!injected && path.basename(filePath) === "AGENTS.md") {
          injected = true;
          concurrentWrite = atomicWriteTextIfUnchanged(
            filePath,
            content,
            expectation,
            mode,
  ...
  if (concurrentWrite === undefined)
    assert.fail("concurrent writer did not start");
...
test("use never plans a missing agent file from another writer's quarantine window", async () => {
  ...
  const planned = await readRegularFile(agentPath, {
    containmentRoot: initial.repositoryRoot,
  });
  ...
  assert.deepEqual(
    (await readdir(initial.repositoryRoot)).filter((name) =>
      name.startsWith(".AGENTS.md."),
    ),
    [],
  );
  const database = openConnection(databasePath);
...
test("use never adopts a concurrent agent result with retained quarantine cleanup", async () => {
  ...
  assert.equal(peerOutcome.cleanupFailures.length, 1);
  const retained = (await readdir(initial.repositoryRoot)).filter(
    (name) => name.startsWith("..AGENTS.md.") && name.endsWith(".cleanup"),
  );
  assert.equal(retained.length, 1);
  ...
  const retainedName = retained[0];
  if (retainedName === undefined)
    assert.fail("retained cleanup artifact is missing");
  await unlink(path.join(initial.repositoryRoot, retainedName));
});
    ...
    planned.mode,
    {
      beforeCleanup: async (artifactPath) => {
        if (artifactPath.includes(".previous."))
          throw new Error("retain cleanup for identity swap");
      },
    },
  );
  assert.equal(peerOutcome.cleanupFailures.length, 1);
  const retainedName = (await readdir(initial.repositoryRoot)).find(
    (name) => name.startsWith("..AGENTS.md.") && name.endsWith(".cleanup"),
  );
  if (retainedName === undefined)
    assert.fail("retained cleanup artifact is missing");
...
test("post-CAS convergence rejects an exact agent result with retained cleanup", async () => {
  ...
  const retainedName = retained[0];
  if (retainedName === undefined)
    assert.fail("retained post-CAS cleanup artifact is missing");
  await unlink(path.join(root, retainedName));
});

test(
  "use never adopts an exact concurrent agent target while a hard-link alias remains",
  {
      ...
      useRepository(
        { root, databasePath },
        {
          atomicWriteTextIfUnchanged: async (
            ...
            if (
              !bindingInjected &&
              path.basename(filePath) === ".kiokuko.json"
            ) {
              bindingInjected = true;
              const proposed = JSON.parse(content) as Record<string, unknown>;
              await writeFile(
...
test(
  "initial binding convergence never adopts a replacement repository-root identity",
  {
    skip: process.platform === "win32",
  },
  async () => {
    ...
    assert.equal(bindingAttempts, 1);
    assert.equal(
      await readFile(path.join(replacementRoot, ".kiokuko.json"), "utf8"),
      winnerBinding,
    );
...
test(
  "initial binding convergence pins a custom agent-file parent across the retry",
  {
    skip: process.platform === "win32",
  },
  async () => {
    ...
    await assert.rejects(
      useRepository(
        { root, databasePath, agentFile: "nested/AGENTS.md" },
        {
          atomicWriteTextIfUnchanged: async (
            filePath,
            content,
...
test("use never rolls back exact concurrent binding and agent-file results it does not own", async () => {
  ...
  const registrationFailure = new Error(
    "registration failed after exact convergence",
  );
  await writeFile(agentPath, "human header\n");
...
test(
  "use rejects noncanonical or broadly-permissioned concurrent bindings",
  { skip: process.platform === "win32" },
  async () => {
...
test("use creates version-23 binding, result, and repository metadata", async () => {
  ...
  try {
    assert.equal(
      database
        .prepare(
          "SELECT agent_template_version AS version FROM repositories WHERE repository_id = ?",
        )
        .get<{ version: number }>(result.repositoryId)?.version,
      23,
    );
  } finally {
    database.close();
...
test("use upgrades a version-1 binding and managed block without changing identity or human bytes", async () => {
  ...
  const data = await mkdtemp(path.join(tmpdir(), "kiokuko-data-"));
  const databasePath = path.join(data, "kiokuko-dsh.sqlite3");
  const first = await useRepository({
    root,
    databasePath,
    workspace: "upgrade-workspace",
  });
  const bindingPath = path.join(root, ".kiokuko.json");
  const binding = JSON.parse(await readFile(bindingPath, "utf8")) as Record<
    string,
    unknown
  >;
  await writeFile(
    bindingPath,
    `${JSON.stringify({ ...binding, templateVersion: 1 }, null, 2)}\n`,
  );
  ...
  const upgradedBinding = JSON.parse(await readFile(bindingPath, "utf8")) as {
    repositoryId: string;
    workspace: string;
  ...
  try {
    const row = database
      .prepare(
        "SELECT repository_id AS repositoryId, workspace, agent_template_version AS version FROM repositories WHERE repository_id = ?",
      )
      .get<{ repositoryId: string; workspace: string; version: number }>(
        first.repositoryId,
      );
...
test("use rejects a managed block with no version declaration without partial mutation", async () => {
  ...
  const agentPath = path.join(root, "AGENTS.md");
  const bindingBefore = await readFile(bindingPath, "utf8");
  const unversionedAgent = (await readFile(agentPath, "utf8")).replace(
    /^<!-- kiokuko-template-version: [1-9][0-9]* -->\n/mu,
    "",
  );
  ...
  await assert.rejects(
    useRepository({ root, databasePath }),
    /missing.*template-version declaration/i,
  );
...
test("use rejects future database version metadata and restores force-rebind file writes", async () => {
  ...
  try {
    database
      .prepare(
        "UPDATE repositories SET agent_template_version = 24 WHERE repository_id = ?",
      )
...
test("use preserves human content and rejects malformed markers", async () => {
  ...
  await import("node:fs/promises").then(({ writeFile }) =>
    writeFile(path.join(root, "AGENTS.md"), "human\n"),
  );
  const data = await mkdtemp(path.join(tmpdir(), "kiokuko-data-"));
  ...
  await assert.rejects(
    useRepository({ root, databasePath: path.join(data, "db.sqlite3") }),
    /malformed/i,
  );
...
test("use rejects duplicate binding keys before database or agent-file mutation", async () => {
  ...
  const duplicateBinding =
    '{"schemaVersion":1,"repositoryId":"repo_first","repositoryId":"repo_second","workspace":"project:duplicate","agentFile":"AGENTS.md","templateVersion":8}\n';
  ...
  for (const dryRun of [true, false]) {
    await assert.rejects(
      useRepository({
        root,
        databasePath,
        agentFile: "missing/nested/AGENTS.md",
        dryRun,
      }),
      (error: unknown) =>
        error instanceof Error &&
...
test(
  "use rejects an intermediate symlink escape before writing outside the repository",
  { skip: process.platform === "win32" },
  async () => {
...
test(
  "use rejects a replaced or symlinked planned agent parent and rolls back every owned file",
  {
    skip: process.platform === "win32",
  },
  async () => {
    for (const variant of ["directory", "symlink"] as const) {
      ...
      const outside = await mkdtemp(
        path.join(tmpdir(), "kiokuko-agent-parent-outside-"),
      );
      const databasePath = path.join(data, "db.sqlite3");
      const initial = await useRepository({ root, databasePath });
      const bindingPath = path.join(initial.repositoryRoot, ".kiokuko.json");
      ...
      await assert.rejects(
            ...
            atomicWriteTextIfUnchanged: async (
              ...
              if (!replaced && path.resolve(filePath) === newAgentPath) {
                ...
                if (variant === "directory") {
                  await mkdir(parentPath);
                  await writeFile(
                    path.join(parentPath, "concurrent.txt"),
                    "concurrent owner\n",
                  );
                } else {
        ...
        (error: unknown) =>
          ...
          "code" in error &&
          error.code ===
            (variant === "directory" ? "CONFLICT" : "SECURITY_REJECTION"),
      );
      ...
      if (variant === "directory") {
        assert.equal(
          await readFile(path.join(parentPath, "concurrent.txt"), "utf8"),
          "concurrent owner\n",
        );
      } else {
        await assert.rejects(access(path.join(outside, "AGENTS.md")));
...
test(
  "use rejects an agentFile hard-link alias of the repository binding",
  { skip: process.platform === "win32" },
  async () => {
...
test(
  "use rejects a previous agentFile hard-link alias of the repository binding before relocation",
  { skip: process.platform === "win32" },
  async () => {
...
test("agent relocation rejects a previous target inside another atomic quarantine", async () => {
  ...
  const bindingBefore = await readFile(bindingPath, "utf8");
  const planned = await readRegularFile(previousPath, {
    containmentRoot: initial.repositoryRoot,
  });
...
test("use preserves files and aggregates close failure after an uncertain COMMIT outcome", async () => {
  ...
  await assert.rejects(
    useRepository(
      { root, databasePath },
      {
        openConnection: openConnectionWithCommittedCommitFailure(
          commitFailure,
          closeFailure,
        ),
      },
  ...
  try {
    assert.equal(
      database
        .prepare(
          "SELECT repository_id AS repositoryId FROM repository_locations WHERE repository_id = ?",
        )
...
test("use rollback preserves a byte-identical binding replaced by another actor", async () => {
  ...
  assert.equal(
    JSON.parse(await readFile(bindingPath, "utf8")).schemaVersion,
    1,
  );
  const database = openConnection(databasePath);
  try {
...
test("force-rebind replaces the exact existing identity and rejects no-op force", async () => {
  ...
  await assert.rejects(
    useRepository({
      root,
      databasePath,
      repositoryId: "repo_unforced_replacement",
    }),
    /force-rebind/i,
  );
  assert.equal(await readFile(bindingPath, "utf8"), bindingBefore);
  ...
  assert.match(
    await readFile(agentPath, "utf8"),
    /Workspace: `project:rebind-after`/u,
  );
  assert.doesNotMatch(
    await readFile(agentPath, "utf8"),
    /project:rebind-before/u,
  );
...
test("existing-binding transitions reject byte-identical concurrent agent replacement before registration", async () => {
    ...
    useRepository(
      ...
      },
      {
        ...
        registerRepositoryAndLocation: () => {
          registrationCalls += 1;
          throw new Error(
            "registration must not run after an agent CAS conflict",
          );
        },
      },
    ),
  ...
  assert.match(
    await readFile(agentPath, "utf8"),
    /Workspace: `project:exact-agent-after`/u,
  );
  const database = openConnection(databasePath);
  try {
    assert.equal(
...
test("force-rebind can move one exact location to an existing repository identity", async () => {
  ...
  assert.match(
    await readFile(path.join(sourceRoot, "AGENTS.md"), "utf8"),
    new RegExp(target.repositoryId, "u"),
  );
...
test(
  "changing agentFile removes only the old managed block and preserves human bytes and mode",
  { skip: process.platform === "win32" },
  async () => {
    ...
    assert.equal(
      (
        await readFile(newPath, "utf8").then(
          (content) => content.match(/BEGIN KIOKUKO MANAGED BLOCK/g) ?? [],
        )
      ).length,
      1,
    );
    ...
    assert.equal(
      repeated.agentFile,
      path.join(repeated.repositoryRoot, "config", "AGENTS.md"),
    );
    assert.equal(await readFile(oldPath, "utf8"), "human-owned\n\n\n");
  },
);
...
test(
  "changing agentFile deletes a managed-only old file and restores it on later failure",
  { skip: process.platform === "win32" },
  async () => {
    ...
    useRepository(
      ...
      },
      {
        unlinkRegularFileIfUnchanged: async (
          ...
          const outcome = await unlinkRegularFileIfUnchanged(
            filePath,
            expectation,
            dependencies,
          );
...
test("changing agentFile with no-agent-file still removes the retired managed-only target", async () => {
  ...
  assert.equal(
    (
      JSON.parse(await readFile(path.join(root, ".kiokuko.json"), "utf8")) as {
        agentFile: string;
      }
    ).agentFile,
    "SKIPPED.md",
...
test(
  "no-agent-file validates the prospective target after retiring the old managed file",
  {
    skip: process.platform === "win32",
  },
  async () => {
...
test("changing agentFile fails closed on a concurrent old-file edit and rolls back owned writes", async () => {
  ...
  assert.equal(
    await readFile(oldPath, "utf8"),
    "concurrent human replacement\n",
  );
  await assert.rejects(access(newPath));
});

test(
  "changing agentFile rejects malformed or symlinked old ownership before mutation",
  { skip: process.platform === "win32" },
  async () => {
    for (const variant of ["malformed", "symlink"] as const) {
      ...
      await assert.rejects(
        useRepository({
          root,
          databasePath,
          agentFile: "replacement/AGENTS.md",
        }),
      ...
      if (variant === "symlink") {
        assert.equal(
          await readFile(path.join(data, "outside.md"), "utf8"),
          "outside-owned\n",
        );
      }
    }
