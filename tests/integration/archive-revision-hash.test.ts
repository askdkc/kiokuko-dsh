...
function structuredV3Scope(): JsonObject {
  return { schemaVersion: 3, visibility: 'project', retrievalScope: 'project-only' };
}
...
function archiveWithLegacyHash(content: string, tags: string[]): string {
  ...
  const tagLines = tags.map((tag) => ({ type: 'tag', entry_id: entry.id, tag }));
...
test('doctor rejects every post-migration legacy tag hash preimage', async () => {
  ...
  try {
    ...
    assert.deepEqual(exactReport.checks.revisionHashes, { ok: false, count: 1 });
    assert.deepEqual(forgedReport.checks.revisionHashes, { ok: false, count: 1 });
  } finally {
    exact.db.close();
    forged.db.close();
  }
...
test('memory archive rejects noncanonical revision hashes without a legacy branch', async () => {
  ...
  try {
    ...
    await writeFile(archivePath, archiveWithLegacyHash(canonicalArchive, LEGACY_TAG_ORDER), 'utf8');
