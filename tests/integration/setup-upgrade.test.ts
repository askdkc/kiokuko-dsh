...
test("setup backs up an existing database before applying pending migrations", async () => {
  ...
  await Promise.all(
    [home, oldMigrations, currentMigrations].map((directory) =>
  ...
  await writeFile(
    path.join(currentMigrations, "002_upgrade.sql"),
    "ALTER TABLE setup_upgrade_data ADD COLUMN upgraded INTEGER NOT NULL DEFAULT 1;\n",
  );
  ...
  try {
    ...
    database
      .prepare("INSERT INTO setup_upgrade_data (id, value) VALUES (1, ?)")
      .run("preserved");
  } finally {
    database.close();
  }
  ...
  try {
    assert.equal(
      backup
        .prepare("SELECT value FROM setup_upgrade_data WHERE id = 1")
    ...
    assert.equal(
      backup
        .prepare(
          "SELECT 1 AS present FROM pragma_table_info('setup_upgrade_data') WHERE name = 'upgraded'",
        )
        .get(),
      undefined,
    );
  ...
  try {
    assert.equal(
      upgraded
        .prepare("SELECT value, upgraded FROM setup_upgrade_data WHERE id = 1")
        .get<{ value: string; upgraded: number }>()?.upgraded,
      1,
    );
  } finally {
    upgraded.close();
...
test("setup rejects a future-version database before writing client configuration", async () => {
  ...
  await Promise.all(
    [home, migrations].map((directory) =>
      mkdir(directory, { recursive: true }),
    ),
  );
  await writeFile(
    path.join(migrations, "001_initial.sql"),
    "CREATE TABLE future_setup_data (id INTEGER PRIMARY KEY);\n",
  );
  const databasePath = path.join(data, "kiokuko", "kiokuko-dsh.sqlite3");
  const database = openConnection(databasePath);
  try {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      /newer/i.test((error as Error).message),
  );
