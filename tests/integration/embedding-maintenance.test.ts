...
async function database(prefix: string) {
  const directory = await mkdtemp(
    path.join(tmpdir(), `kiokuko-embedding-${prefix}-`),
  );
...
test("workspace export omits derived embedding rows and import enqueues regeneration", async () => {
  ...
  try {
    const entry = recordEmbeddedEntry(
      source.db,
      workspace,
      "entry-embedding-archive",
    );
    ...
    assert.deepEqual(
      ...
      ["audit", "checksum", "entry", "manifest"],
    );
    assert.doesNotMatch(
      archive,
      /embedding_profiles|entry_embeddings|embedding_jobs|query_embeddings/u,
    );
    ...
    assert.deepEqual(
      listEmbeddingJobs(target.db).map((job) => ({
        entryId: job.entryId,
        state: job.state,
      })),
    ...
    assert.equal(
      target.db
        .prepare("SELECT COUNT(*) AS count FROM entry_embeddings")
...
test("full SQLite backup preserves vectors and restored doctor detects later corruption", async () => {
  ...
  const entry = recordEmbeddedEntry(
    source.db,
    "project:embedding-backup",
    "entry-embedding-backup",
  );
  try {
  ...
  try {
    ...
    const mismatched = inspectEmbeddingHealth(
      restored,
      {
        ...environment,
    ...
    restored
      .prepare("UPDATE entry_embeddings SET vector_hash = ? WHERE entry_id = ?")
      .run("c".repeat(64), entry.id);
