...
import {
  answerAgentTask,
  prepareAgentTask,
} from "../../src/akinator/agent-task.js";
...
const NO_MEMORY_POLICY = {
  memoryReasoningRequired: false,
  contextWithheld: false,
  withheldReason: null,
} as const;
const AVAILABLE_MEMORY_POLICY = {
...
async function repository(prefix: string): Promise<string> {
  const root = await mkdtemp(
    path.join(tmpdir(), `kiokuko-memory-policy-${prefix}-`),
  );
...
async function createDatabase(prefix: string) {
  const directory = await mkdtemp(
    path.join(tmpdir(), `kiokuko-memory-policy-db-${prefix}-`),
  );
...
function exposeRepositoryBindingMutation(
  ...
  database
    .prepare("UPDATE repositories SET last_used_at = ? WHERE repository_id = ?")
    .run("2000-01-01T00:00:00.000Z", repositoryId);
  database
    .prepare(
      "UPDATE repository_locations SET last_seen_at = ? WHERE repository_id = ? AND canonical_root = ?",
    )
  ...
  `,
    ...
    .get<{ lastUsedAt: unknown; lastSeenAt: unknown }>(
      repositoryId,
      repositoryRoot,
    );
...
function rejectedAnswerStateWithoutLocations(
  database: SqliteDatabase,
): Record<string, unknown> {
...
test("withholds actionable memory but continues the repair task when memory-reasoning is unavailable", async () => {
  ...
  try {
    ...
    const missingRecommendation = missing.capabilities.recommendations.find(
    ...
    assert.deepEqual(
      missing.memoryPolicy,
      MISSING_MEMORY_POLICY_WITH_ONE_STORED_ENTRY,
    );
    ...
    assert.equal(
      database
        ...
        .get<{ count: number }>(missing.run.runId)?.count,
      0,
    );
    ...
    const availableRecommendation = available.capabilities.recommendations.find(
      (item) => item.name === "memory-reasoning",
    );
...
test("fresh default setup supplies the exact memory capability that unlocks build and debug delivery", async () => {
  ...
  const environmentRoot = await mkdtemp(
    path.join(tmpdir(), "kiokuko-memory-policy-setup-"),
  );
  ...
  const databasePath = path.join(data, "kiokuko", "kiokuko-dsh.sqlite3");
  ...
  assert.equal(
    setup.files.some(
      (file) => file.path === installedSkillPath && file.action === "created",
    ),
    true,
  );
  ...
  try {
    ...
    for (const [taskType, target] of [
      ...
      const missing = await prepareAgentTask(database, {
        ...
        client: {
          kind: "test",
          sessionId: `default-setup-${taskType}-missing`,
        },
        skillDiscoveryMode: "off",
      });
      assert.equal(missing.context, null);
      ...
      const availableInput = {
        ...
        client: {
          kind: "test",
          sessionId: `default-setup-${taskType}-available`,
        },
      ...
      assert.equal(available.nextAction, "proceed");
      assert.equal(
        available.context?.items.some((item) => item.entryId === entry.id),
        true,
      );
      ...
      assert.equal(
        database
          ...
          .get<{ count: number }>(available.run.runId)?.count,
        1,
      );
  ...
  try {
    ...
    assert.equal(
      prepared.capabilities.recommendations.some(
        (item) => item.name === "memory-reasoning" && item.required === true,
      ),
      false,
    );
    assert.equal(
      prepared.context?.items.some((item) => item.entryId === curated.id),
      true,
    );
  } finally {
    database.close();
...
test("pre-discovery memory gate rejects a concurrent ledger profile revision instead of reusing the stale build decision", async () => {
  ...
  const { database, concurrent } = await createDatabasePair(
    "preview-profile-race",
  );
  let revised = false;
  let networkCalls = 0;
  try {
    ...
    const hooked = new Proxy(database, {
      get(target, property) {
        ...
        return (sql: string) => {
          ...
          if (!sql.includes("SELECT workspace FROM entries WHERE id = ?"))
            return statement;
          return new Proxy(statement, {
            get(statementTarget, statementProperty) {
              ...
              return (...args: unknown[]) => {
                if (!revised) {
                  const row = concurrent
                    .prepare(
                      "SELECT run_id AS runId FROM ledger_runs ORDER BY created_at DESC LIMIT 1",
                    )
                    .get<{ runId: string }>();
                  assert.ok(row);
                  revised = true;
    ...
    await assert.rejects(
      prepareAgentTask(hooked, {
        ...
        profileHints: {
          taskType: "build",
          target: "preview profile race sentinel",
          expected: "tests pass",
          constraints: null,
        },
        capabilities: [SOUL_CAPABILITY],
        ...
        fetchImpl: async () => {
          networkCalls += 1;
          throw new Error("discovery must remain behind the memory gate");
        },
      }),
      (error: unknown) =>
...
test("task_answer rejects a replacement capability catalog before mutating intake", async () => {
  ...
  try {
    ...
    exposeRepositoryBindingMutation(
      database,
      prepared.project.repositoryId,
      prepared.project.repositoryRoot,
    );
    const before = rejectedAnswerState(database);
    await assert.rejects(
      answerAgentTask(database, {
        cwd: root,
...
test("task_answer rejects a changed manifest before mutating the bound intake run", async () => {
  ...
  try {
    ...
    const beforeAnswers = database
      .prepare(
        "SELECT COUNT(*) AS count FROM akinator_answers WHERE session_id = ?",
      )
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        ...
        error.code === "CONFLICT" &&
        error.message ===
          "Project manifest differs from the snapshot bound when the run was opened",
    );
    ...
    assert.equal(
      database
        .prepare("SELECT COUNT(*) AS count FROM repository_fingerprints")
        .get<{ count: number }>()?.count,
      0,
    );
    assert.deepEqual(rejectedAnswerState(database), before);
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        ...
        error.code === "CONFLICT" &&
        error.message ===
          "Skill discovery request differs from the request bound when the run was opened",
    );
...
test("reordered capability descriptors replay the same task run", async () => {
  ...
  const firstCatalog = [
    SOUL_CAPABILITY,
    {
      ...
      description: "Reason about stored memory.",
    },
    {
      kind: "skill",
  ...
  try {
    const first = await prepareAgentTask(database, {
      ...request,
      capabilities: firstCatalog,
    });
    const replayed = await prepareAgentTask(database, {
      ...request,
      capabilities: [...firstCatalog].reverse(),
    });
...
test("requestId identifies one logical task request and conflicts on changed bound input", async () => {
  ...
  const request = {
    ...
    profileHints: {
      ...
      expected: "one run per logical request",
      constraints: null,
    },
  ...
  try {
    ...
    assert.equal(
      JSON.stringify(
        database.prepare("SELECT * FROM gateway_idempotency").all(),
      ).includes("memory-policy-logical-request"),
      false,
    );
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        ...
        error.code === "CONFLICT" &&
        error.message ===
          "Task context request differs from the request bound when the run was opened",
    );
    assert.deepEqual(rejectedAnswerState(database), before);
...
test("task_answer rejects a cwd rebound to another repository without touching either binding", async () => {
  ...
  try {
    ...
    const reboundLocation = database
      .prepare(
        `
    ...
    `,
      ...
      .get<{ repositoryId: unknown; lastSeenAt: unknown }>(
        prepared.project.repositoryRoot,
      );
      ...
      get(target, property) {
        ...
        return (sql: string) => {
          if (
            !rebound &&
            sql.includes(
              "SELECT lr.run_id AS runId FROM ledger_runs AS lr JOIN run_intakes",
            )
          ) {
            concurrent
              .prepare(
...
test("delivers a Japanese checkpoint policy to a Japanese-only migration task", async () => {
  ...
  try {
    ...
    assert.equal(
      prepared.context?.items.some((item) => item.entryId === entry.id),
      true,
    );
...
test("withholds actionable memory before external Skill discovery and continues the task", async () => {
  ...
  try {
    const seed = await prepareAgentTask(database, {
      ...
      profileHints: {
        taskType: "build",
        target: "Svelte beacon component",
        expected: "tests pass",
        constraints: null,
      },
      capabilities: [SOUL_CAPABILITY],
...
test("exact task_prepare replay uses current helpful feedback for the bound weak delivery", async () => {
  ...
  try {
    ...
    const first = await prepareAgentTask(database, request);
    const delivered = first.context?.items.find(
      (item) => item.entryId === entry.id,
    );
    assert.ok(delivered);
    assert.equal(
      delivered.selectionReasons.includes("literal_fallback_match"),
      true,
    );
    ...
    assert.ok(
      replay.capabilities.recommendations.some(
        (item) =>
...
test("exact task_prepare replay reranks when new actionable ordinary memory appears", async () => {
  ...
  try {
    ...
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM context_delivery_entries WHERE entry_id = ?",
        )
        .get<{ count: number }>(actionable.id)?.count,
      0,
    );
  } finally {
    database.close();
...
test("exact task_prepare replay gates the current ledger-revised profile", async () => {
  ...
  try {
    ...
    const request = {
      ...
      profileHints: {
        taskType: "research" as const,
        target: "current profile replay sentinel",
...
test("exact task_prepare replay rejects a run completed by memory_checkpoint", async () => {
  ...
  try {
    ...
    await assert.rejects(
      prepareAgentTask(database, request),
      (error: unknown) =>
        error instanceof Error &&
        "code" in error &&
        error.code === "CONFLICT" &&
    ...
    assert.equal(
      database
        ...
        .get<{ status: string }>(first.run.runId)?.status,
      "completed",
    );
    assert.equal(
      database
...
test("discovery rolls back external imports when the task run becomes terminal during source fetch", async () => {
  ...
  try {
    ...
    assert.equal(
      database
        .prepare("SELECT status FROM ledger_runs")
    ...
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM entries WHERE created_by = 'kiokuko-skill-discovery'",
        )
        .get<{ count: number }>()?.count,
      0,
    );
...
test("defers malformed manifest parsing while intake needs an answer and canonicalizes the effective MCP client", async () => {
  ...
  const request = {
    ...
    fetchImpl: async () => {
      networkCalls += 1;
      throw new Error("discovery must wait for intake");
    },
  };
  try {
    const omittedClient = await prepareAgentTask(database, request);
    const explicitDefaultClient = await prepareAgentTask(database, {
      ...request,
      client: { kind: "mcp" },
    });
...
test("exposes only code-point-bounded scoped context for prepared memory", async () => {
  ...
  try {
    ...
    assert.equal(
      Array.from(prepared.context?.items[0]?.bodyPreview ?? "").length,
      2,
    );
