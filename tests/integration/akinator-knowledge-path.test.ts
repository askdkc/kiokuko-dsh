...
test("counts only verified independent Akinator runs and makes repeated portable knowledge skill-ready", async () => {
  ...
  try {
    ...
    assert.equal(
      database
        .prepare(
          "SELECT COUNT(*) AS count FROM akinator_reasoning_paths WHERE qualified = 1",
        )
        .get<{ count: number }>()?.count,
      2,
    );
    const curated = await curateMemoryCandidates(database, {
      workspace,
      skillReadyOnly: true,
    });
    assert.equal(
      curated.candidates.length,
      1,
      "same generalized concept is shown once",
    );
...
test("does not qualify retrieval-free runs without fresh verification or a passing test", async () => {
  ...
  try {
    ...
    const row = database
      .prepare(
        "SELECT qualified, disqualification_reasons_json FROM akinator_reasoning_paths",
      )
      .get<{ qualified: number; disqualification_reasons_json: string }>();
    assert.equal(row?.qualified, 0);
    assert.match(
      row?.disqualification_reasons_json ?? "",
      /no-fresh-verification-or-passing-test/u,
    );
  } finally {
    database.close();
  }
