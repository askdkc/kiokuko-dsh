...
async function fixture() {
  ...
  const reusable = recordEntry(database, {
    ...
    summary:
      "A reusable workflow for safely recovering from migration failures.",
...
test("curator identifies reusable candidates and globalizes only after explicit service call", async () => {
  ...
  try {
    for (let index = 0; index < 5; index += 1)
      recallEntries(data.database, {
        workspace: "project:curator-test",
        query: "SQLite migration recovery",
      });
    const listed = await curateMemoryCandidates(data.database, {
      workspace: "project:curator-test",
    });
    assert.deepEqual(
      listed.candidates.map((candidate) => candidate.entryId),
      [data.reusable.id],
    );
    ...
    assert.equal(
      candidate.knowledge.qualifiedHits,
      0,
      "retrieval frequency is not a qualified hit",
    );
    assert.equal(candidate.knowledge.skillReady, false);
    assert.equal(candidate.overview.length, 3);
    assert.match(
      candidate.overview[1],
      /troubleshooting workflow for the target project/u,
    );
    ...
    assert.ok(
      candidate.draft.changes.includes("project-references-normalized"),
    );
    assert.ok(candidate.draft.changes.includes("paths-generalized"));
    assert.doesNotMatch(
      JSON.stringify(candidate.draft),
      /(?:This project|\/Users\/example|repo_curator_test|project:curator-test)/u,
    );
    ...
    assert.equal(
      result.global.provenance.reference,
      `${data.reusable.id}@${data.reusable.revision}#${CURATOR_DRAFT_VERSION}`,
    );
    ...
    assert.doesNotMatch(
      JSON.stringify({
        ...
        summary: result.global.summary,
        body: result.global.body,
      }),
      /(?:This project|\/Users\/example|repo_curator_test|project:curator-test)/u,
    );
    ...
    recordEntry(
      data.database,
      {
        ...
        scope: buildStructuredScope({
          ...
          portableReason: "forged claimant",
        }),
        provenance: {
      ...
      recordEntry(
        data.database,
        {
          ...
          scope: buildStructuredScope({
            visibility: "global",
            retrievalScope: "global",
            portableReason: "duplicate claimant",
          }),
          provenance: {
            type: "curator_globalize",
            reference: `${data.reusable.id}@${data.reusable.revision}#${CURATOR_DRAFT_VERSION}`,
...
const corruptStructuredMetadata: Array<{
  field: "applicability" | "signals";
  value: JsonValue;
}> = [
...
for (const corrupt of corruptStructuredMetadata) {
  test(`curator fails closed on malformed stored ${corrupt.field} metadata`, async () => {
    ...
    try {
      ...
      const corruptHash = canonicalEntryRevisionContentHash({
        ...corruptEntry,
        scope: corruptScope,
      });
      ...
      data.database
        .prepare(
          "UPDATE entry_revisions SET scope_json = ?, content_hash = ? WHERE entry_id = ? AND revision = ?",
        )
        .run(
          canonicalJson(corruptScope),
          corruptHash,
          corruptEntry.id,
          corruptEntry.revision,
        );

      const isIntegrityError = (error: unknown) =>
        (error as { code?: unknown }).code === "INTEGRITY_ERROR";
      await assert.rejects(
        curateMemoryCandidates(data.database, {
          workspace: "project:curator-test",
        }),
        isIntegrityError,
      );
...
test("curator treats unversioned reserved-key collisions as unstructured legacy scope", async () => {
  ...
  try {
    ...
    const result = await curateMemoryCandidates(data.database, {
      workspace: legacy.workspace,
    });

    const candidate = result.candidates.find(
      (item) => item.entryId === legacy.id,
    );
    assert.ok(candidate);
...
test("web exposes curator candidates and a revision-checked globalize action", async () => {
  ...
  try {
    ...
    const listedResponse = await fetch(
      `${web.url}/api/curator/candidates?workspace=project%3Acurator-test`,
      { headers },
    );
    ...
    const listed = (await listedResponse.json()) as {
      candidates: Array<{
        entryId: string;
        revision: number;
        draft: { body: string; version: string };
      }>;
    };
    ...
    assert.doesNotMatch(
      listed.candidates[0]?.draft.body ?? "",
      /\/Users\/example/u,
    );

    const globalizedResponse = await fetch(
      `${web.url}/api/curator/globalize?workspace=project%3Acurator-test`,
      {
        method: "POST",
    ...
    const globalized = (await globalizedResponse.json()) as {
      global: { workspace: string; body: string };
    };
...
test("web curator lists candidates across projects and globalizes the selected source workspace", async () => {
  ...
  try {
    ...
    const headers = { cookie: cookie as string };
    const listedResponse = await fetch(
      `${web.url}/api/curator/candidates?limit=50`,
      { headers },
    );
    assert.equal(listedResponse.status, 200);
    const listed = (await listedResponse.json()) as {
      workspace: string | null;
      candidates: Array<{ entryId: string; workspace: string }>;
    };
    assert.equal(listed.workspace, null);
    assert.deepEqual(
      new Set(listed.candidates.map((candidate) => candidate.workspace)),
      new Set(["project:curator-test", "project:other-curator-test"]),
    );
    assert.ok(
      listed.candidates.some(
        (candidate) =>
          candidate.entryId === other.id &&
          candidate.workspace === "project:other-curator-test",
      ),
    );
    ...
    assert.equal(globalizedResponse.status, 200);
    const globalized = (await globalizedResponse.json()) as {
      global: { workspace: string; provenance: { sourceWorkspace: string } };
    };
    assert.equal(globalized.global.workspace, "global");
    assert.equal(
      globalized.global.provenance.sourceWorkspace,
      "project:other-curator-test",
    );
  } finally {
    await web.close();
    data.database.close();
...
test("web curator applies SQL-backed filters and exposes facets", async () => {
  ...
  try {
    ...
    const facetsResponse = await fetch(
      `${web.url}/api/curator/facets?workspace=all`,
      { headers },
    );
    assert.equal(facetsResponse.status, 200);
    const facets = (await facetsResponse.json()) as {
      facets: {
        projects: Array<{ workspace: string }>;
        tags: Array<{ value: string }>;
        memoryClasses: Array<{ value: string }>;
      };
    };
    assert.ok(
      facets.facets.projects.some(
        (facet) => facet.workspace === "project:curator-test",
      ),
    );
    assert.ok(
      facets.facets.tags.some((facet) => facet.value === "skill:database"),
    );
    assert.ok(
      facets.facets.memoryClasses.some(
        (facet) => facet.value === "troubleshooting",
      ),
    );

    const filteredResponse = await fetch(
      `${web.url}/api/curator/candidates?workspace=all&tag=skill%3Adatabase&memoryClass=troubleshooting&search=SQLite`,
      { headers },
    );
