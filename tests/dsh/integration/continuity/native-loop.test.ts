...
for (const continuity of ['off', 'shadow', 'active'] as const) {
const mode = 'research'
test(`real DSH continuity ${continuity}: final requests after compaction and same-run reload`, {
  ...
  try {
    ...
    const lastRequest = JSON.stringify(model.requests.at(-1))
    assert.match(lastRequest, /根拠|acquisition/)
    if (continuity === 'active') {
      assert.match(lastRequest, /Continuity \(host projection/)
      assert.match(lastRequest, /Current request presentation is unknown/)
      const observations = adapter.host.efficiency!.snapshot().continuity
      assert.ok(observations.length > 0)
      assert.ok(observations.every((value: any) => value.bytes <= 4096 && value.copiesInRequest === 1))
    } else assert.doesNotMatch(lastRequest, /Continuity \(host projection/)
    ...
    t.diagnostic(JSON.stringify({ continuity, modelRequests: model.requests.length, reads, writes,
      requestBytes: model.requests.map((request: any) => Buffer.byteLength(JSON.stringify(request))),
      durationMs: performance.now() - testStarted, providerTokens: null, mockUsageIsNotRealUsage: true }))
