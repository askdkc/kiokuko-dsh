...
for (const mode of ['off', 'observe', 'active'] as const) test(`native preStep and boundary worker: ${mode}, new failure, repeated call, compaction, invalidation and terminal`, {
  ...
  let calls = 0, beforeFull = 0, runId = '', agent: any
  ...
  try {
    ...
    await waitFor(() => {
      ...
      return agent.status === 'idle' && agent.session.snapshotEvents().some((e: any) => e.type === 'assistant/message' && JSON.stringify(e.data).includes('REFRESH_FIXTURE_COMPLETE'))
    ...
    assert.equal(db.prepare("SELECT status FROM enno_contracts WHERE run_id=?").get(runId)?.status, 'completed')
